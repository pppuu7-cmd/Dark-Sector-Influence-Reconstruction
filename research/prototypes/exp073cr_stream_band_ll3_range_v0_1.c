...
/*
 * NON-AUTHORITATIVE RESEARCH PROTOTYPE v0.1.
 *
 * Closer control-flow analogue of the frozen Exp073CA helper than v0.0:
 * retain OpenMP setup, one-thread parallel region, per-thread buffers,
 * critical error assignment, and a one-element omp-for band loop.
 * Only the inner ll3 output domain and output storage are sliced.
 *
 * nthreads MUST equal 1. Resource parallelism belongs outside this helper.
 */
int exp073cr_stream_compress_band_ll3_range_v0_1(const char *nmtlib_path,
...
#pragma omp parallel default(none) shared(pcl_mask,lmax,s1,s2,n1,n2,out,nls,lstart,drc3jj,error_code,edges,nb,ib,ll3_lo,ll3_hi)
  {
...
#pragma omp critical
      ...
      /* Preserve the frozen omp-for band scaffold with one selected band. */
#pragma omp for schedule(dynamic)
      for(int jb=ib;jb<ib+1;jb++) {
        int lo=edges[jb];
        int hi=edges[jb+1];
        double *acc=out;
        for(int ll2=lo;ll2<hi;ll2++) {
          if(ll2<lstart)
            continue;
          /* Only this inner output loop is restricted. */
