#!/usr/bin/env python3
"""NON-AUTHORITATIVE Exp073CR 64-shard scheduler prototype v0.1.

Supersedes research v0.0, which bound a nonexistent helper symbol and was never
executed. This file remains preflight-only: no scientific/resource authority.
"""
...
OUTER_WORKERS = 8
NESTED_THREADS = 1
HELPER_SYMBOL = 'exp073cr_stream_compress_band_ll3_range_v0_1'
...
def load_candidate(path: Path):
    ...
    shards=[]; alloc=c['allocation_by_band']; bounds=c['ll3_boundaries_by_band']
...
def bind_helper(helper_so: Path):
    lib=ctypes.CDLL(str(helper_so.resolve()))
    try:
        f=getattr(lib,HELPER_SYMBOL)
    except AttributeError as e:
        raise RuntimeError(f'helper ABI missing required symbol {HELPER_SYMBOL}') from e
    dptr=ctypes.POINTER(ctypes.c_double); iptr=ctypes.POINTER(ctypes.c_int)
    # C ABI: nmtlib,pcl,lmax,s1,s2,n1,n2,edges,nb,band,ll3_lo,ll3_hi,nthreads,out
    f.argtypes=[ctypes.c_char_p,dptr,ctypes.c_int,ctypes.c_int,ctypes.c_int,ctypes.c_int,ctypes.c_int,
                iptr,ctypes.c_int,ctypes.c_int,ctypes.c_int,ctypes.c_int,ctypes.c_int,dptr]
...
def store_shard(root: Path, s: Shard, a, candidate_sha: str, tel: dict):
    ...
    r={'format':'DSIR_LL3_SHARD_PROTOTYPE_V0_1','complete':True,'band':s.band,'ll3_interval':[s.ll3_lo,s.ll3_hi],
       'shape':[s.ll3_hi-s.ll3_lo],'dtype':'<f8','payload_sha256':ahash(a),'candidate_sha256':candidate_sha,
       'helper_symbol':HELPER_SYMBOL,'outer_workers':8,'nested_threads':1,**tel}
...
def worker(s: Shard, pcl_path: str, helper_so: str):
    ...
    f,dptr,iptr=bind_helper(Path(helper_so))
...
def diagnostic(root: Path, stage: str, exc: BaseException, admitted, remaining, branch=None, script=None):
    rec={'format':'DSIR_LL3_SHARD_DIAGNOSTIC_PROTOTYPE_V0_1','stage':stage,'complete':True,
...
def compute(root: Path, candidate: Path, pcl: Path, helper: Path, branch: str, sync_script: Path):
    _,shards,candidate_sha=load_candidate(candidate); exact_array(pcl,(L,),'prototype PCL')
    # ABI probe is intentionally fail-fast before spawning workers.
    bind_helper(helper)
    ...
    try:
        ...
        diagnostic(root,'compute',e,admitted,remaining,branch,sync_script); raise
...
def assemble_band(root: Path, candidate: Path, band: int):
    ...
    for s in sorted(ss,key=lambda z:z.ll3_lo):
        if load_shard(root,s,candidate_sha) is None: raise RuntimeError(f'band {band}: missing shard {s}')
    ...
    if out[0]!=0.0 or out[1]!=0.0 or not np.all(np.isfinite(out)): raise RuntimeError(f'band {band}: assembled invariant failed')
    return canon(out)
...
def main():
    ...
    if a.cmd=='validate-candidate':
        c,s,h=load_candidate(Path(a.candidate)); out={'candidate_sha256':h,'shards':len(s),'bands':c['bands'],'helper_symbol':HELPER_SYMBOL}
        if a.helper: bind_helper(Path(a.helper)); out['helper_abi']='PASS'
