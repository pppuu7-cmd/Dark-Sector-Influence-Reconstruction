...
- G7/G8/G9 = OPEN.
- reproducibility/provenance/governance/infrastructure QA and individual angular-authority PASSes add **0 scientific-readiness points**.
...
## Historical authority and negative records — preserve exactly

- Exp073X: `INCOMPLETE_INFRASTRUCTURE_RESOURCE_CANCELLED_NO_AUTHORITY_REUSE`.
- historical Exp073X2R primary P: hosted exact repeatability PASS / +0 readiness; Wm_S0 SHA `6ec29f6dbbcf0c29d7af9d6eb98d271bcd06e00d28cebe658b5e266f7ef18d0f`.
...
## Controlled single-thread Wm_S0 authority

Original Exp073AI completed both expensive replicas but its original aggregator failed before comparison because of an environment-receipt path error. Preserve it as:

`INCOMPLETE_INFRASTRUCTURE_AGGREGATOR_ENV_PATH_ERROR_BEFORE_REPEATABILITY_CLASSIFICATION`.

Exp073AM prospectively repaired only aggregation while reusing immutable replicas and unchanged exact comparator:

- run `33321661835`, job `99284585530`;
- artifact `9735051043`;
...
Therefore global bitwise identity is execution-route-sensitive, but the controlled route is internally exact-reproducible.
...
Exp073AP applied that frozen classifier to the real state:

- run `33324664267`, job `99292571445`;
- artifact `9735869454`;
- digest `sha256:8c60618717777a3c913053bcd5437c0ab548e294e98b72b9c5c869dcc52caacf`;
- real decision `AUTHORIZE_EXECUTION_QUALIFIED_EXACT_SUCCESSOR_ROUTE`.

This is governance/reproducibility authority only and +0 readiness.

Controlled-route Wm_S0 anchor:

`8ac59fc060195addcc5cd8b6d75e32fbc6dbfeea8456f4c83e8bf0cf034b9220`.

Every future replica must set exactly:
...
- unchanged Exp073AA physical/angular executor `45ed8d8d1e90cdaf314e0384b6f3cdfef369925b`;
...
- no final comparator authority artifact yet.

The initial comparator commit `64ae1eae3fc8902c0ec4368c2b7209be4fcbc67e` had a pre-freeze JSON-selection harness mistake and is not authority. It was detected and superseded before trigger/output; the exact admission criterion did not change.

After both valid replicas reach the comparator, allowed classes are only:
...
8. after Layer A/B PASS: covariance/whitening, representation/resolvability-controlled nuisance geometry, quotient/relation/null, then fresh G8 after G7 relation freeze.

Until the real complete pre-support operator manifest exists, strict Article-3 readiness remains **52%**.
