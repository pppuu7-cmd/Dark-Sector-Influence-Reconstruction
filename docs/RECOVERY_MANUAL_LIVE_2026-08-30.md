...
Latest inspection: both remain inside `Compute exact single-thread replica`. Do not duplicate AI.
...
## Exp073AL — AI-vs-primary exact cross-route stability firewall

Exp073AL was prospectively frozen while Exp073AI remained active and before any AI output existed. It addresses a question intentionally separate from AI's own internal PASS criterion: whether a future valid AI PASS also reproduces the already hosted primary-P exact canonical authority.
...
- workflow freeze `642263e96f545d8f3035af73adecf0af8449e7b8`;
- trigger/head `2bd07bfdc850bc83897c6d67d9b13b583003aed9`;
- hosted run `33319102300`, job `99277802521` = success;
- artifact `9734352248`;
- digest `sha256:a9ed4f0be6ed4dd25924658697b76b5f0b016d0b7cdc09feafe3aa0441278aa1`;
- token `PASS_EXP073AL_AI_VS_PRIMARY_EXACT_STABILITY_CLASSIFIER_SYNTHETIC_V0_1`.

Classification: `HOSTED_SYNTHETIC_EXACT_STABILITY_CLASSIFIER_PASS_NON_SCIENTIFIC_PLUS_0_READINESS`.
...
- valid AI PASS with canonical SHA exactly equal to primary-P SHA `6ec29f6dbbcf0c29d7af9d6eb98d271bcd06e00d28cebe658b5e266f7ef18d0f` -> `EXACT_CROSS_ROUTE_STABILITY_AI_EQUALS_PRIMARY_P`;
- valid AI PASS with a different valid canonical SHA -> `DETERMINISTIC_SINGLE_THREAD_ROUTE_BUT_EXACT_AUTHORITY_SHIFT_FROM_PRIMARY_P`.

Thus a future AI internal PASS cannot be over-interpreted as cross-route exact stability unless its canonical authority exactly matches historical primary P. Conversely, an AI internal PASS with a different hash remains an internal AI PASS but is a negative cross-route exact-stability result.

Exp073AL hard-fails primary-P identity drift, malformed hashes, invalid completion/token/hash combinations, unknown fields, and any leakage activation. It does not read angular array values, AJ2 environment labels, support, covariance, nuisance geometry or G8; cannot release production; and adds +0 readiness.

## Current authorized route

`Exp073AI active -> Exp073AK2 terminal/control-plane classification -> immutable hosted AI token only if valid -> AJ2 independent environment-provenance label when both receipts exist -> if AI validly PASSes, Exp073AL exact identity check against primary P -> only then consider a separately prospectively frozen succession rule`.
...
5. `recovery/2026-08-30_exp073al_hosted_pass_ai_still_running.md`
...
9. `experiments/073al_article3_ai_vs_primary_exact_stability_classifier_v0_1_prereg.md`
10. `experiments/073ah2_article3_q_repeatability_forensic_binding_v0_2_prereg.md`
11. `experiments/073af_article3_x2_to_exp073aa_release_control_v0_1_prereg.md`
12. `experiments/073ag_article3_exact_14window_authority_aggregator_schema_v0_1_prereg.md`
13. `experiments/073ae_article3_presupport_authority_join_schema_v0_1_prereg.md`.
