...
| Designer f(R) B0 background/AP null | **HARD PASS / Exp.038** | run `32785800977`: source-proven `EFTwDE=0 -> w=-1`; exact saved background/AP zero for `B0=0..1e-3` |
...
| Low-z AP geometry atlas coverage | **SUBSTANTIALLY CLOSED** | C0 origin, C1/C2 hard nonzero directions, C3/C5 hard exact-null cells; C4 WDM remains intentionally in separate small-scale block until a geometry contract is defined |
...
| CAMB printed growth-log precision | **LIMIT / REJECTED FOR TANGENTS** | four-decimal `sigma8` and `sigma8^2_vd/sigma8` quantize small-B0 response; machine-readable transfer/cross-power output required |
...
| GDM solver/manifold | PASS + calibrated two-axis patch | P cs2/cv2 nearly collinear; AP exact-null; temporal growth still only `1.334 deg`; slip is needed |
| H-EFTCAMB designer f(R) | **MG-S0 + MG-S1 + Exp.038 PASS** | exact GR limit, common-baseline production, source-proven LCDM background and exact B0 AP-null |
...
| New residual law | **OPEN G7** | observation-space growth/shape/window kernels and rank stability still incomplete |
| Withheld prediction | **OPEN G8** | mandatory before discovery |

## Current comparison findings

1. **Exact channel-null structure now repeats across two qualitatively different families.** GDM `cs2/cv2` and designer-f(R) `B0` are exactly background/AP-null in their frozen constructions while remaining perturbation-active. This supports block-sparse influence trajectories, but is not a universal law.
2. **Degeneracies migrate between operators rather than disappearing monotonically.** Smooth-w vs IDE alpha is strongly separated in AP (`72.80 deg`) but nearly degenerate in finite-bin temporal growth (`10.31 deg`), while IDE alpha vs GDM changes in the opposite direction (~`25 deg` full structure to ~`61 deg` temporal growth).
3. **IDE alpha/beta forms a useful three-channel ladder:** AP `9.04 deg` -> temporal growth `29.40 deg` -> full structure `58.93 deg`. Temporal evolution restores some mechanism information lost in geometry, but not all.
4. **GDM cs2/cv2 remains a hard microphysical degeneracy in density/growth:** `0.323 deg` raw low-k structure and `1.334 deg` finite-bin growth. Metric slip remains the established independent separator.
5. **GDM/f(R) scale lookalikes are separated by time:** leading scale-only modes are `0.078-0.102 deg`, finite-bin growth `16.05-17.28 deg`, full `(k,z)` structure `25.18-25.49 deg`. The full history is more informative than growth-only compression.
6. The finite-node ShapeFit `m+n` proxy is not universal: about `36%` representation residual for GDM/f(R), so proxy angles are not DESI distinguishability claims.
7. The exact AP identity proves anchored `r_E` does not discard AP information: additive log-E normalization cancels in `F_AP=E chi`.
8. The printed H-EFTCAMB growth summaries are too coarsely rounded for small-`B0` tangent work. The next RSD step must use high-precision density/velocity transfer or cross-power products.
...
1. Turn Experiment 039 from a definition contract into a numerical density-velocity representability gate. Use machine-readable transfer variables, not printed growth summaries.
2. For C5 H-EFTCAMB, preserve high-precision transfer outputs carrying `delta_tot`, `v_newtonian_cdm`, and `v_newtonian_baryon`; construct a validated total-matter/tracer velocity convention before forming `S_dd`, `S_dTheta`, `S_ThetaTheta`.
3. For C3 GDM and C2 IDE, create matched pinned-solver transfer-output runs with the same gauge/total-matter contract. Do not mix solver velocity conventions silently.
4. Compute the ShapeFit-rescaled smoothing radius `R=s*8 h^-1 Mpc` and the RSD representability defect before admitting a scalar `f_sigma_s8` response.
...
6. Then form the corrected joint ShapeFit block `Z=C^{-1/2} Delta O` and compare raw-theory versus data-whitened geometry.
7. Stress-test local/global response spectra under family priors `pi`, channel removal, solver precision, and within-family sampling before any `R_model(pi)` statement.
8. In parallel, continue observational lensing/slip and small-scale-transfer blocks because the hard discriminant graph identifies them as independent high-value separators.
