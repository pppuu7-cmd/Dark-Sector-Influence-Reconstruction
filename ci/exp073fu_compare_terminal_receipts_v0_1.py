...
def main():
    ...
    # Infrastructure-only schema/name transform from the frozen FS comparator.
    # Keep arithmetic, equality/finiteness scoring and provenance checks unchanged.
    repl=[
        ('exp073fs','exp073fu'),
        ('EXP073FS','EXP073FU'),
        ('ww-s1-s2','ww-s1-s3'),
    ...
    for t in (
        ...
        'FAIL_EXP073FU_WW_S1_S3_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1',
    ):
        if t not in src: raise RuntimeError(f'fail-closed missing FU comparator invariant {t!r}')
    stale=('ww-s1-s2','ww_s1_s2','WW_S1_S2','S1->S2','[1,2]',"'s2':1")
