...
SCHEMA='dsir.exp073cv.wm_s3.production_exact_adapter.omp8.v0.3'
EXPECTED_TEAM=8


def run_downstream_omp8(exe:Path, inp:Path, full_path:Path, ncls:int, nb:int, nl:int):
...
def execute(args):
    ...
    base.run_downstream=run_downstream_omp8
    try:
