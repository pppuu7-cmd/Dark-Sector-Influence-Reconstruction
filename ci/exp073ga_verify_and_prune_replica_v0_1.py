...
def main():
 ...
 for t in ("'ordered_source_indices':[3,3]","'source_pair':'S3->S3'","'same_field_object_handoff':True",'PASS_EXP073GA_REPLICA_'):
  if t not in s: raise RuntimeError(f'fail-closed missing GA pruner invariant {t!r}')
 ...
 exec(compile(s,'exp073ga_verify_and_prune_replica_v0_1.transformed.py','exec'),{'__name__':'__main__','__file__':str(BASE)})
