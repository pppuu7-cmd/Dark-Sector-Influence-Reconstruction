...
: "${R1_ARTIFACT_ID:?}" "${R1_DIGEST:?}" "${NAMASTER_HEAD:?}" "${PATCH_BLOB:?}"
: "${DRIVER_V01_BLOB:?}" "${DRIVER_V02_BLOB:?}" "${TASKRUNNER_BLOB:?}" "${EM_GENERATOR_BLOB:?}" "${EM_COMPARE_BLOB:?}"
SCI_ROOT="$HOME/.cache/dsir/exp073fa-ww-s0-s2-filebacked-ab-v0-1"
CHECKPOINT_ROOT="$SCI_ROOT/checkpoints"
STOCK_ENV="$HOME/.cache/dsir-nmt27"
PATCH_ENV="$HOME/.cache/dsir-nmt27-filebacked-read-v0-2"
PATCH_SRC="$HOME/.cache/dsir/namaster-v2.7-filebacked-read-src-v0-2"
PATCH_FILE="$GITHUB_WORKSPACE/patches/namaster-v2.7-dsir-filebacked-mcm-read-v0.2.patch"
ACT_ROOT="$SCI_ROOT/local_exp073em_activation"
EXPECTED_MCM_BYTES=19327352832
EXPECTED_PATCH_BLOB='d534b698f9131688d263eedcef27260386c58641'
mkdir -p "$SCI_ROOT" "$CHECKPOINT_ROOT" "$HOME/.cache/dsir"
exec 9>"$HOME/.cache/dsir/DSIR-HOME-PC.exp073fa-ww-s0-s2.lock"
flock -n 9 || { echo BLOCKED_EXP073FA_LOCK; exit 4; }
...
test "$PATCH_BLOB" = "$EXPECTED_PATCH_BLOB"
...
grep -F 'diff --git a/src/nmt_io.c b/src/nmt_io.c' "$PATCH_FILE" >/dev/null
grep -F 'dsir_nmt_alloc_unbinned(w,(int)n_el,0);' "$PATCH_FILE" >/dev/null
...
if bad: raise SystemExit('BLOCKED_EXP073FA_COMPETING_SELF_HOSTED '+repr(sorted(set(bad))))
PY
done
echo PASS_EXP073FA_LIVE_EXCLUSIVITY
...
test "$cpus" = 8 || { echo "RESOURCE_BLOCKED_EXP073FA_CPU_AFFINITY=$cpus"; exit 4; }
for v in OPENBLAS_NUM_THREADS MKL_NUM_THREADS NUMEXPR_NUM_THREADS BLIS_NUM_THREADS VECLIB_MAXIMUM_THREADS; do test "${!v:-1}" = 1 || { echo "BLOCKED_EXP073FA_NESTED_THREADS $v=${!v}"; exit 4; }; done
test "${OMP_NUM_THREADS:-8}" = 8 || { echo "BLOCKED_EXP073FA_OMP_NUM_THREADS=${OMP_NUM_THREADS:-unset}"; exit 4; }

min_free=$((50*1024*1024*1024)); wsl_free="$(df -B1 --output=avail "$HOME"|tail -n1|tr -d ' ')"; test "$wsl_free" -ge "$min_free" || { echo "RESOURCE_BLOCKED_EXP073FA_WSL_FREE=$wsl_free"; exit 4; }
host_free="$(powershell.exe -NoProfile -Command '[int64](Get-PSDrive -Name C).Free'|tr -d '\r'|tail -n1|tr -d ' ')"; [[ "$host_free" =~ ^[0-9]+$ ]] || exit 4; test "$host_free" -ge "$min_free" || { echo "RESOURCE_BLOCKED_EXP073FA_WINDOWS_FREE=$host_free"; exit 4; }

TELEMETRY="$SCI_ROOT/resource_telemetry.log"
(while true; do echo "===== $(date --iso-8601=seconds) ====="; free -h||true; df -h "$HOME"||true; find "$SCI_ROOT/mmap" -type f -name 'dsir-nmt-mcm-*' -printf '%p %s bytes\n' 2>/dev/null||true; ps -eo pid,ppid,stat,etimes,cmd|grep -E 'Runner.Worker|exp073fa|dsir-nmt-mcm'|grep -v grep||true; sleep 30; done) >>"$TELEMETRY" 2>&1 & MONPID=$!
trap 'kill "$MONPID" 2>/dev/null || true; wait "$MONPID" 2>/dev/null || true' EXIT

MF="$HOME/.cache/dsir-miniforge"; test -x "$MF/bin/conda" || { echo BLOCKED_EXP073FA_NO_MINIFORGE; exit 4; }
...
PY
BUILD_MARKER="$PATCH_ENV/.dsir_exp073fa_filebacked_read_build_identity"; expected_marker="NAMASTER_HEAD=$NAMASTER_HEAD PATCH_SHA256=$PATCH_SHA256"
...
# Exact local storage activation qualifier; no WW result is inspected here.
...
run_replica(){
 ...
 echo "=== Exp073FA replica $rep start $(date --iso-8601=seconds) ===" | tee -a "$log"
 DSIR_NMT_FILEBACKED_MCM=1 DSIR_NMT_MMAP_DIR="$mm" "$PATCH_PY" ci/exp073fa_ww_s0_s2_durable_ab_production_v0_2.py --replica "$rep" --r1-root "$R1_ROOT" --r1-artifact-digest "$R1_DIGEST" --checkpoint-root "$CHECKPOINT_ROOT" --source-head "$FROZEN_SOURCE_HEAD" --contract-fingerprint "$CONTRACT_FP" --ab-out "$SCI_ROOT/ab_compare.json" 2>&1 | tee -a "$log"
...
out={'schema':'dsir.exp073fa.post_receipt_prune.v0.1','replica':rec['replica'],'selected_ee_sha256':h,'preserved_complete_receipt':True,'pruned_only_after_receipt':True}; (r/'post_receipt_prune.json').write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
PY
 rm -f "$SCI_ROOT/mmap/$rep"/dsir-nmt-mcm-* || true
}
run_replica A; prune_replica A
run_replica B; prune_replica B
DSIR_NMT_FILEBACKED_MCM=1 DSIR_NMT_MMAP_DIR="$SCI_ROOT/mmap/compare" "$PATCH_PY" ci/exp073fa_ww_s0_s2_durable_ab_production_v0_2.py --replica AB --r1-root "$R1_ROOT" --r1-artifact-digest "$R1_DIGEST" --checkpoint-root "$CHECKPOINT_ROOT" --source-head "$FROZEN_SOURCE_HEAD" --contract-fingerprint "$CONTRACT_FP" --ab-out "$SCI_ROOT/ab_compare.json" | tee "$SCI_ROOT/ab_compare_stdout.txt"
