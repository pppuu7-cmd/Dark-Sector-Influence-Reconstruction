...
tmp="$RUNNER_TEMP/exp073fm_home_filebacked_fullres_v0_1.transformed.sh"
BASE="$BASE" OUT="$tmp" python3 - <<'PY'
import os
from pathlib import Path
base=Path(os.environ['BASE']); out=Path(os.environ['OUT']); s=base.read_text(encoding='utf-8')
required_repl=[
 ('exp073fa','exp073fm'),('Exp073FA','Exp073FM'),('EXP073FA','EXP073FM'),
 ('ww_s0_s2','ww_s1_s1'),('ww-s0-s2','ww-s1-s1'),('S0->S2','S1->S1'),('[0,2]','[1,1]'),
]
for old,new in required_repl:
    if old not in s: raise SystemExit(f'fail-closed missing frozen home transform token {old!r}')
    s=s.replace(old,new)
s=s.replace('WW_S0_S2','WW_S1_S1')
required=[
 'ci/exp073fm_ww_s1_s1_durable_ab_production_v0_1.py',
 'ci/exp073fm_ww_s1_s1_durable_ab_production_v0_2.py',
 'exp073fm-ww-s1-s1-filebacked-ab-v0-1',
]
for token in required:
    if token not in s: raise SystemExit(f'fail-closed missing Exp073FM home invariant {token!r}')
...
pos=s.index(marker)
tail='''run_replica A
"$PATCH_PY" ci/exp073fm_verify_and_prune_replica_v0_1.py --checkpoint-root "$CHECKPOINT_ROOT" --replica A | tee "$SCI_ROOT/A_prune_verify.log"
rm -f "$SCI_ROOT/mmap/A"/dsir-nmt-mcm-* || true
run_replica B
"$PATCH_PY" ci/exp073fm_verify_and_prune_replica_v0_1.py --checkpoint-root "$CHECKPOINT_ROOT" --replica B | tee "$SCI_ROOT/B_prune_verify.log"
rm -f "$SCI_ROOT/mmap/B"/dsir-nmt-mcm-* || true
"$PATCH_PY" ci/exp073fm_compare_terminal_receipts_v0_1.py --root "$SCI_ROOT" --out "$SCI_ROOT/ab_compare.json" | tee "$SCI_ROOT/ab_compare_stdout.txt"
cp "$SCI_ROOT/ab_compare.json" "$SCI_ROOT/terminal_receipt.json"
'''
s=s[:pos]+tail
for token in ('ci/exp073fm_verify_and_prune_replica_v0_1.py','ci/exp073fm_compare_terminal_receipts_v0_1.py','terminal_receipt.json'):
