...
/*
 * Exp073CA checkpoint-safe range wrapper.
 *
 * Scientific arithmetic inside each band is intentionally identical to the
 * frozen Exp073BW exp073bw_stream_compress inner loop.  The sole architecture
 * change is that only a contiguous band range [ib_lo,ib_hi) is returned, so a
 * caller can persist completed canonical rows between chunks without ever
 * checkpointing a partial band.
 */
int exp073ca_stream_compress_range(const char *nmtlib_path,
                                   ...
                                   int nthreads,double *out)
{
  ...
  if(drc3jj==NULL)
    return 90;
  if((nthreads<1)||(nb<1)||(ib_lo<0)||(ib_hi<=ib_lo)||(ib_hi>nb)) {
    dlclose(handle);
    return 91;
  }
  ...
  int nout=ib_hi-ib_lo;
  memset(out,0,(size_t)nout*(size_t)nls*sizeof(double));
  omp_set_dynamic(0);
  omp_set_num_threads(nthreads);
  int error_code=0;

#pragma omp parallel default(none) shared(pcl_mask,lmax,s1,s2,n1,n2,out,nls,lstart,drc3jj,error_code,edges,nb,ib_lo,ib_hi)
  {
...
#pragma omp for schedule(dynamic)
      for(int ib=ib_lo;ib<ib_hi;ib++) {
        int lo=edges[ib];
        int hi=edges[ib+1];
        double *acc=out+(size_t)(ib-ib_lo)*(size_t)nls;
