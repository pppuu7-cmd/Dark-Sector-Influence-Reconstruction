...
def main() -> None:
    ...
    ap.add_argument('--replica', choices=['A', 'B'], required=True)
    ...
    source, source_meta = source_count_map(one(r1root, 'exp073r1_v05_bin0_pixel_indices_le_u32.bin'))
    lens, lens_meta = lens_mask(Path(args.lens_mask))
    te, workspace_meta = get_te_window(lens, source)

    pass_token = f'PASS_EXP073X2_REPLICA_{args.replica}_DES_N4096_WM0_MASK_ONLY_V0_1'
    result = {
        'experiment': 'Exp073X2',
        ...
        'next_authorized_step': 'Only after both independent replicas exist, compare them in the preregistered lightweight Exp073X2 aggregator.',
    }
