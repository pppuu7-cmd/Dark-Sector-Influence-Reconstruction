...
WRAPPER = Path(__file__).with_name('exp073fg_ww_s0_s3_durable_ab_production_v0_1.py')
spec = importlib.util.spec_from_file_location('exp073fg_wrapper_v01', WRAPPER)
if spec is None or spec.loader is None:
    raise RuntimeError('cannot load frozen Exp073FG v0.1 wrapper')
wrap = importlib.util.module_from_spec(spec)
spec.loader.exec_module(wrap)
mod = wrap.load_transformed()
