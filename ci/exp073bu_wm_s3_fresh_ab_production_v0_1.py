...
from exp073bu_fresh_wm_s3_pcl_v0_1 import LMAX, PCL_SHAPE, canonical_f8_sha, reconstruct_lens_mask, reconstruct_s3_count_map
...
def load_manifest(root:Path,stage:str,replica:str,source_head:str,contract_fingerprint:str):
    ...
    ok=(rec.get('complete') is True and rec.get('stage')==stage and rec.get('replica')==replica and rec.get('checkpoint_namespace')==NAMESPACES[replica] and rec.get('source_head')==source_head and rec.get('contract_fingerprint')==contract_fingerprint and rec.get('historical_wm_s3_numerical_import') is False and rec.get('other_replica_output_read') is False)
    if not ok: raise RuntimeError('fail-closed checkpoint identity mismatch')
    return rec

def fresh_or_restore_masks(root:Path,replica:str,r1_root:Path,lens_path:Path,source_head:str,contract_fingerprint:str):
    st=load_manifest(root,'fresh_masks_complete',replica,source_head,contract_fingerprint); lp=root/'lens_mask.npy'; sp=root/'s3_mask.npy'
    if st is None:
        lens,lens_rec=reconstruct_lens_mask(lens_path); source,s3_rec=reconstruct_s3_count_map(r1_root); lsha=atomic_npy(lp,lens); ssha=atomic_npy(sp,source)
        stage_manifest(root,'fresh_masks_complete',replica,source_head,contract_fingerprint,{'lens_mask':{'canonical_sha256':lsha,'shape':list(lens.shape)},'s3_mask':{'canonical_sha256':ssha,'shape':list(source.shape)},'lens_authority':lens_rec,'s3_authority':s3_rec,'reconstruction_counts':{'lens':1,'source':1}})
...
def validated_finished_receipt(root:Path,replica:str,source_head:str,contract_fingerprint:str):
    st=load_manifest(root,'replica_receipt_complete',replica,source_head,contract_fingerprint); rp=root/'replica_receipt.json'
    if st is None: return None
    if not rp.exists() or file_sha(rp)!=st['payloads']['replica_receipt']['sha256']: raise RuntimeError('fail-closed final receipt restore mismatch')
    rec=json.loads(rp.read_text()); te=Path(rec['selected_te_path'])
    if not te.exists() or file_sha(te)!=rec['selected_te_sha256'] or rec['selected_te_sha256']!=st['payloads']['selected_te']['sha256']: raise RuntimeError('fail-closed final TE restore mismatch')
    return rec
...
def run_replica(replica:str,args):
    ...
    done=validated_finished_receipt(root,replica,args.source_head,args.contract_fingerprint)
    if done is not None: return done
    workspace_path=root/'fresh_workspace.fits'; ws=load_manifest(root,'fresh_workspace_mcm_complete',replica,args.source_head,args.contract_fingerprint)
    if ws is None:
        ...
        # One field pair only; identical objects feed fresh PCL and fresh stock workspace.
        ...
        pcl_sha=atomic_npy(root/'fresh_mask_pcl.npy',pcl)
        b=nmt.NmtBin.from_edges(BAND_EDGES[:-1],BAND_EDGES[1:]); w=nmt.NmtWorkspace(); w.compute_coupling_matrix(f0,f2,b); w.write_to(str(workspace_path)); wsha=file_sha(workspace_path)
        stage_manifest(root,'fresh_workspace_mcm_complete',replica,args.source_head,args.contract_fingerprint,{'workspace_fits':{'sha256':wsha},'fresh_pcl':{'canonical_sha256':pcl_sha},'same_field_object_handoff':True,'field_object_ids':[id(f0),id(f2)],'reconstruction_counts':recon})
        del w,b,f0,f2,lens,source,pcl; gc.collect()
    else:
        if not workspace_path.exists() or file_sha(workspace_path)!=ws['payloads']['workspace_fits']['sha256']: raise RuntimeError('fail-closed workspace restore SHA mismatch')
        pcl_sha=ws['payloads']['fresh_pcl']['canonical_sha256']; recon={'lens':0,'source':0}; wsha=ws['payloads']['workspace_fits']['sha256']
    stage_manifest(root,'mcm_fits_verified',replica,args.source_head,args.contract_fingerprint,{'workspace_fits':{'sha256':wsha}})
    te_st=load_manifest(root,'selected_te_complete',replica,args.source_head,args.contract_fingerprint); full_st=load_manifest(root,'full_window_complete',replica,args.source_head,args.contract_fingerprint)
    full_path=root/'exact_route'/'full_window.bin'; te_path=root/'exact_route'/'selected_te.bin'
    if te_st is not None:
        if not te_path.exists() or file_sha(te_path)!=te_st['payloads']['selected_te']['sha256']: raise RuntimeError('fail-closed selected TE restore mismatch')
        if full_st is None or not full_path.exists() or file_sha(full_path)!=full_st['payloads']['full_window']['sha256']: raise RuntimeError('fail-closed full-window restore mismatch')
        adapter_rec={'status':'RESTORED_FROM_VERIFIED_SELECTED_TE_CHECKPOINT','historical_wm_s3_numerical_import':False}
    else:
        edges_path=root/'edges.json'; edges_path.write_text(json.dumps(BAND_EDGES.tolist()))
        ad=argparse.Namespace(workspace_fits=str(workspace_path),edges_json=str(edges_path),ncls=2,nl=12288,emulator=args.downstream_exe,out_dir=str(root/'exact_route'),source_head=args.source_head,contract_fingerprint=args.contract_fingerprint,checkpoint_namespace=NAMESPACES[replica],component_blobs_json=str(Path(args.component_blobs_json)))
        adapter_rec=execute_exact_adapter(ad)
...
def compare_replicas(a:dict,b:dict,out_path:Path):
    aa=np.memmap(Path(a['selected_te_path']),dtype='<f8',mode='r',shape=(39,12288)); bb=np.memmap(Path(b['selected_te_path']),dtype='<f8',mode='r',shape=(39,12288)); sha_equal=a['selected_te_sha256']==b['selected_te_sha256']; array_equal=bool(np.array_equal(aa,bb)); del aa,bb
    ...
    rec={'schema':SCHEMA+'.ab_compare','status':status,'sha256_equal':sha_equal,'numpy_array_equal':array_equal,'a_sha256':a['selected_te_sha256'],'b_sha256':b['selected_te_sha256'],'no_tolerance_rescue':True,'science_gate_scored':True}; atomic_json(out_path,rec); return rec
