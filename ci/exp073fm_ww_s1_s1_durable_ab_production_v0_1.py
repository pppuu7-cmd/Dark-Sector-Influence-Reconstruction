...
NAMESPACES = {'A':'checkpoints/exp073fm-ww-s1-s1-a-v0-1','B':'checkpoints/exp073fm-ww-s1-s1-b-v0-1'}
...
def load_base():
    ...
    spec = importlib.util.spec_from_file_location('exp073fm_base_fa', BASE)
...
def install_s1s1(mod):
    ...
    def load_manifest(root,stage,replica,head,fp):
        ...
        if not ok:
            raise RuntimeError('fail-closed WW_S1_S1 checkpoint identity mismatch')
        return r

    def mmap_payload_sha(p:Path):
        a=mod.np.load(p,mmap_mode='r',allow_pickle=False)
        if a.dtype.str!='<f8' or tuple(a.shape)!=(12*4096*4096,):
            raise RuntimeError('fail-closed S1 source restore geometry')
    ...
    def fresh_or_restore_sources(root,replica,r1_root,r1_digest,head,fp):
        ...
        p1=root/'s1_count_map.npy'
        if st is None:
            auth=mod.validate_r1(r1_root,r1_digest)
            s1,m1=mod.source_count_map(r1_root,1)
            h1=mod.atomic_npy(p1,s1)
            del s1
            gc.collect()
            st=manifest(root,'fresh_sources_complete',replica,head,fp,{'s1_count_map':{'canonical_sha256':h1,'shape':[12*4096*4096],'dtype':'<f8'},'r1_authority':auth,'s1_authority':m1,'ordered_source_indices':[1,1],'reconstruction_counts':{'s1':1},'same_source_map_both_sides':True})
        if not p1.exists():
            raise RuntimeError('fail-closed missing S1 source checkpoint payload')
        if mmap_payload_sha(p1)!=st['payloads']['s1_count_map']['canonical_sha256']:
            raise RuntimeError('fail-closed S1 source restore SHA mismatch')
        if st['payloads'].get('ordered_source_indices')!=[1,1] or st['payloads'].get('reconstruction_counts')!={'s1':1} or st['payloads'].get('same_source_map_both_sides') is not True:
            raise RuntimeError('fail-closed S1 same-source semantics')
        return p1,st
    ...
    def strict_validated_finished(root,replica,head,fp):
        ...
        for stage in CHECKPOINT_ORDER:
            ...
            stages[stage]=st
        p1=root/'s1_count_map.npy'; wp=root/'fresh_workspace.fits'; full=root/'exact_route'/'full_window.bin'; ee=root/'exact_route'/'selected_ee.bin'; rp=root/'replica_receipt.json'
        for p in (p1,wp,full,ee,rp):
            if not p.is_file():
                raise RuntimeError(f'fail-closed missing complete-stage payload {p.name}')
        src=stages['fresh_sources_complete']['payloads']
        if mmap_payload_sha(p1)!=src['s1_count_map']['canonical_sha256'] or src.get('ordered_source_indices')!=[1,1] or src.get('reconstruction_counts')!={'s1':1}:
            raise RuntimeError('fail-closed S1 source complete-stage payload')
        ...
        if r.get('source_pair')!='S1->S1' or r.get('ordered_source_indices')!=[1,1] or r.get('same_field_object_handoff') is not True or r.get('bpw_route')!='public_get_bandpower_windows_after_filebacked_fits_read':
            raise RuntimeError('fail-closed completed receipt S1S1 public-route semantics')
        return r
    ...
    def run_replica(replica,args):
        ...
        if done is not None:
            return done
        wp=root/'fresh_workspace.fits'
        ws=load_manifest(root,'fresh_workspace_mcm_complete',replica,args.source_head,args.contract_fingerprint)
        if ws is None:
            p1,src=fresh_or_restore_sources(root,replica,Path(args.r1_root),args.r1_artifact_digest,args.source_head,args.contract_fingerprint)
            s1=mod.np.load(p1,mmap_mode='r',allow_pickle=False)
            f1=mod.nmt.NmtField(s1,None,spin=2)
            del s1
            gc.collect()
            fid=id(f1)
            b=mod.nmt.NmtBin.from_edges(mod.BAND_EDGES[:-1],mod.BAND_EDGES[1:])
            w=mod.nmt.NmtWorkspace()
            # Frozen same-object handoff: exactly one field object, passed twice.
            w.compute_coupling_matrix(f1,f1,b)
            w.write_to(str(wp))
            wsha=mod.file_sha(wp)
            ws=manifest(root,'fresh_workspace_mcm_complete',replica,args.source_head,args.contract_fingerprint,{'workspace_fits':{'sha256':wsha},'same_field_object_handoff':True,'ordered_source_indices':[1,1],'field_object_ids':[fid,fid],'field_construction_count':1,'source_map_sha256':[src['payloads']['s1_count_map']['canonical_sha256'],src['payloads']['s1_count_map']['canonical_sha256']]})
            del w,b,f1
            gc.collect()
        else:
            if ws['payloads'].get('same_field_object_handoff') is not True or ws['payloads'].get('ordered_source_indices')!=[1,1] or ws['payloads'].get('field_construction_count')!=1:
                raise RuntimeError('fail-closed restored same-field semantics mismatch')
            ids=ws['payloads'].get('field_object_ids',[])
            if len(ids)!=2 or ids[0]!=ids[1]:
                raise RuntimeError('fail-closed restored same-object identity mismatch')
        ...
        manifest(root,'mcm_fits_verified',replica,args.source_head,args.contract_fingerprint,{'workspace_fits':{'sha256':wsha},'ordered_source_indices':[1,1],'same_field_object_handoff':True})
        ee_st=load_manifest(root,'selected_ee_complete',replica,args.source_head,args.contract_fingerprint)
        full_st=load_manifest(root,'full_window_complete',replica,args.source_head,args.contract_fingerprint)
    ...
    def compare(a,b,out):
        for r in (a,b):
            if r.get('source_pair')!='S1->S1' or r.get('ordered_source_indices')!=[1,1] or r.get('same_field_object_handoff') is not True or r.get('bpw_route')!='public_get_bandpower_windows_after_filebacked_fits_read' or r.get('historical_ww_numerical_import') is not False or r.get('other_replica_output_read') is not False:
                raise RuntimeError('fail-closed A/B S1S1 semantics')
        aa=mod.np.memmap(a['selected_ee_path'],dtype='<f8',mode='r',shape=mod.EE_SHAPE)
        bb=mod.np.memmap(b['selected_ee_path'],dtype='<f8',mode='r',shape=mod.EE_SHAPE)
        se=a['selected_ee_sha256']==b['selected_ee_sha256']
        ae=bool(mod.np.array_equal(aa,bb))
        finite=bool(mod.np.isfinite(aa).all() and mod.np.isfinite(bb).all())
        del aa,bb
        status='PASS_EXP073FM_WW_S1_S1_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1' if se and ae and finite else 'FAIL_EXP073FM_WW_S1_S1_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'
        r={'schema':SCHEMA+'.ab_compare','status':status,'classification':'SCIENTIFIC_CANDIDATE_PASS_PENDING_PROVENANCE_ADMISSION' if status.startswith('PASS_') else 'SCIENTIFIC_FAIL','sha256_equal':se,'numpy_array_equal':ae,'all_finite':finite,'a_sha256':a['selected_ee_sha256'],'b_sha256':b['selected_ee_sha256'],'source_pair':'S1->S1','ordered_source_indices':[1,1],'bpw_route':'public_get_bandpower_windows_after_filebacked_fits_read','same_field_object_handoff':True,'no_tolerance_rescue':True,'historical_ww_numerical_import':False,'other_replica_output_read':False,'science_gate_scored':True,'ww_s1_s1_authority_created':False}
        mod.atomic_json(out,r)
        return r

    mod.manifest=manifest
    mod.load_manifest=load_manifest
    mod.fresh_or_restore_sources=fresh_or_restore_sources
    mod.validated_finished=strict_validated_finished
    mod.run_replica=run_replica
    mod.compare=compare
    return mod


def load_s1s1():
    return install_s1s1(load_base())
