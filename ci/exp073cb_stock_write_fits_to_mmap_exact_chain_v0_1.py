...
def main():
    ...
    rec={'schema':'dsir.exp073cb.stock_write_fits_to_mmap_exact_chain.v0.1','status':status,'accounting':'+0/+0','science_gate_scored':False,'wm_s3_authority_created':False,'pymaster_version':version,'cases':rows,'historical_or_des_data_read':False,'no_tolerance_rescue':True,'get_coupling_matrix_forbidden':True,'causal_repair_of':'Exp073CA v0.1 run 33827950939 C5 infrastructure failure','causal_repair_scope':'self-referential forbidden-token guard only; scientific arithmetic/cases unchanged','operation_route':'fresh stock workspace -> stock write_to FITS -> destroy workspace -> read-only FITS memmap row-stream to canonical <f8 durable input -> Exp073BY mmap full-stock downstream'}
