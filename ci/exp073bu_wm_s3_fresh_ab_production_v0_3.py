...
SCHEMA = 'dsir.exp073bu.wm_s3.fresh_ab_production.v0.3'
...
def _accept_finished_receipt_read_only(root: Path, replica: str, args, existing: dict):
    ...
    if explicit is not None:
        ...
        return existing
    # Legacy complete receipts are immutable historical evidence. Accept them
    # read-only only when both the receipt and independently validated workspace
    # prove the original fresh {1,1} lineage. Never migrate/rewrite them here.
