...
def main():
  ...
  exe=Path(EXPECTED['executor'][0]).read_text()
  dec=Path(EXPECTED['decision'][0]).read_text()
  wf=Path(EXPECTED['blueprint'][0]).read_text()
  # Fail-closed structural checks: launch authority is required in lane and child before Class import.
  ...
  # Execute only response-blind static preflight. No launch authority is passed.
  pre=Path(a.out).with_name('executor_static_preflight.json')
  subprocess.run([
    'python3',EXPECTED['executor'][0],'--mode','static-preflight','--contract',EXPECTED['r1_contract'][0],
    '--plan',a.plan,'--out',str(pre)],check=True)
  ...
  # Prove lane mode fails closed before science when launch authority is absent.
  fail=Path(a.out).with_name('lane_without_authority.txt')
  cp=subprocess.run([
    'python3',EXPECTED['executor'][0],'--mode','lane','--replicate','R01','--contract',EXPECTED['r1_contract'][0],
    '--plan',a.plan,'--out',str(Path(a.out).with_name('must_not_exist_lane.json'))],text=True,capture_output=True)
  ...
  if Path(a.out).with_name('must_not_exist_lane.json').exists(): raise RuntimeError('unauthorized lane produced output')

  subprocess.run(['python3','-m','py_compile',EXPECTED['executor'][0],EXPECTED['decision'][0]],check=True)
  out={
    'schema':'LAYERB_BETA_V0_26_R1_SENTINEL_STATIC_AUDIT_V0_1','classification':'SENTINEL_IMPLEMENTATION_RESPONSE_BLIND_STATIC_AUDIT_PASS',
