...
def main():
 ...
 s=BASE.read_text()
 for old,new in [('exp073fm','exp073fw'),('EXP073FM','EXP073FW'),('ww_s1_s1','ww_s2_s2'),('ww-s1-s1','ww-s2-s2'),('WW_S1_S1','WW_S2_S2'),('S1->S1','S2->S2'),('[1,1]','[2,2]')]:
  if old not in s: raise RuntimeError(f'fail-closed missing FW comparator token {old!r}')
  s=s.replace(old,new)
 for t in ("'source_pair':'S2->S2'","'ordered_source_indices':[2,2]","'same_field_object_handoff':True",'checkpoints/exp073fw-ww-s2-s2-a-v0-1','checkpoints/exp073fw-ww-s2-s2-b-v0-1','PASS_EXP073FW_WW_S2_S2_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'):
  if t not in s: raise RuntimeError(f'fail-closed missing FW comparator invariant {t!r}')
 for t in ("'source_pair':'S1->S1'","'ordered_source_indices':[1,1]",'checkpoints/exp073fm-ww-s1-s1-a-v0-1','checkpoints/exp073fm-ww-s1-s1-b-v0-1','PASS_EXP073FM_WW_S1_S1_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'):
