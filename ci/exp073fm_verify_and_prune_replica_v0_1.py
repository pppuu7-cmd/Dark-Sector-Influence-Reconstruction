...
def main():
    ...
    src=manifests['fresh_sources_complete']['payloads']; p1=r/'s1_count_map.npy'
    if not p1.is_file(): raise RuntimeError('fail-closed missing S1 source payload before prune')
    h1=canon_npy(p1)
    if h1!=src['s1_count_map']['canonical_sha256'] or src.get('ordered_source_indices')!=[1,1] or src.get('reconstruction_counts')!={'s1':1} or src.get('same_source_map_both_sides') is not True:
        raise RuntimeError('fail-closed S1 source payload/order before prune')
    ...
    expected={'replica':a.replica,'source_pair':'S1->S1','ordered_source_indices':[1,1],'same_field_object_handoff':True,'source_head':SOURCE_HEAD,'contract_fingerprint':CONTRACT,'checkpoint_namespace':ns,'bpw_route':'public_get_bandpower_windows_after_filebacked_fits_read','historical_ww_numerical_import':False,'other_replica_output_read':False,'science_gate_scored':False}
    for k,v in expected.items():
        if rec.get(k)!=v: raise RuntimeError(f'fail-closed terminal receipt identity {k}')
    ad=rec.get('adapter_receipt',{})
    if ad.get('mcm_backing_bytes')!=19327352832 or ad.get('mcm_filebacked') is not True or ad.get('mcm_proc_maps') is not True or ad.get('read_unbinned_MCM') is not True or ad.get('route')!='public_get_bandpower_windows_after_filebacked_fits_read' or ad.get('no_tolerance_rescue') is not True:
        raise RuntimeError('fail-closed file-backed adapter evidence')
    out={'schema':'dsir.exp073fm.post_receipt_prune.v0.1','replica':a.replica,'checkpoint_namespace':ns,'source_head':SOURCE_HEAD,'contract_fingerprint':CONTRACT,'selected_ee_sha256':hee,'replica_receipt_sha256':hrp,'complete_chain_verified_before_prune':True,'preserved_complete_receipt':True,'pruned_only_after_receipt':True,'stage_manifest_sha256':manifest_sha,'verified_payload_sha256':{'s1_count_map':h1,'workspace_fits':hwp,'full_window':hfull,'selected_ee':hee,'replica_receipt':hrp},'ordered_source_indices':[1,1],'source_pair':'S1->S1','same_field_object_handoff':True,'no_tolerance_rescue':True}
    q=r/'post_receipt_prune.json'; q.write_text(json.dumps(out,indent=2,sort_keys=True)+'\n')
    for p in (p1,wp,full): p.unlink()
    print(f'PASS_EXP073FM_REPLICA_{a.replica}_FULL_CHAIN_VERIFIED_BEFORE_PRUNE_V0_1')
