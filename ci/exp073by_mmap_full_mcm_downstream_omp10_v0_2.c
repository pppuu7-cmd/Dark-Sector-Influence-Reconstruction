...
int main(int argc,char **argv){
  ...
  /* Parallelize only independent output cells. The l2/l3 accumulation order
     inside each scalar is intentionally identical to serial v0.1. */
  #pragma omp parallel for collapse(4) schedule(static) num_threads(DSIR_WORKERS)
  ...
  /* Each (icl1,ib1) owns one output row. Inner l1/l2 accumulation order is
     unchanged from serial v0.1, so parallel scheduling cannot reorder a sum. */
  ...
  for(int icl1=0;icl1<ncls;icl1++) for(int ib1=0;ib1<nb;ib1++){
    ...
    for(int l1=edges[ib1];l1<edges[ib1+1];l1++){
      ...
      for(int icl2=0;icl2<ncls;icl2++) for(int l2=0;l2<L;l2++){
        size_t index_2=(size_t)ncls*l2+icl2; double m0=gsl_matrix_get(mat,index_b1,index_2);
        gsl_matrix_set(mat,index_b1,index_2,m0+m[index_1*nr+index_2]*wf);
      }
    }
  }

  /* LU/inversion and GSL dgemm remain unchanged from serial v0.1. */
