...
def main():
    ...
    ap.add_argument('--jk-authority',required=True)
    ...
    if jk.get('classification')!='COMMON_GRID_SECOND_REFINEMENT_NOT_CONVERGED_PLUS_0_PLUS_0' or jk.get('artifact_verified_independently') is not True:
        raise SystemExit('invalid JK authority')
    if jk.get('artifact_zip_sha256')!=JK_ARTIFACT_SHA or jk.get('observations',{}).get('max_atomic_coarse_vs_fine_relative_component_difference')!=JK_MAX:
        raise SystemExit('JK authority mismatch')
    if jk.get('observations',{}).get('retained_after_layer_b')!=107 or jk.get('observations',{}).get('unsupported_target_evaluations')!=0:
        raise SystemExit('JK support authority mismatch')
    if ji.get('classification')!='GUARD_NODE_FULL_SUPPORT_FEASIBLE_PLUS_0_PLUS_0' or ji.get('geometry',{}).get('stream_sha256')!=JI_GEOM_SHA:
    ...
    jj=load_module('exp073jj_jl_parent',a.jj_script)
    ir=load_module('exp073ir_jl_parent',a.ir_script)
    ...
    if (l2048,u2048,len(coarse))!=(0,1,2049) or (l4096,u4096,len(fine))!=(0,1,4097):
        raise SystemExit('frozen guard construction mismatch')

    jj.ResolutionSuite.coarse_nodes=coarse
    jj.ResolutionSuite.fine_nodes=fine
    jj.ResolutionSuite.audit={}
    scratch=Path(a.scratch)/'third_refinement'
    out_inner=scratch/'exp073jl_inner_ir.json'
    ...
    converged=bool(
        exact_parent and unsupported==0 and lookup<=LOOKUP_REL_TOL
    ...
    classification='COMMON_GRID_THIRD_REFINEMENT_CONVERGED_PLUS_0_PLUS_0' if converged else 'COMMON_GRID_THIRD_REFINEMENT_NOT_CONVERGED_PLUS_0_PLUS_0'
    factor=(JK_MAX/mx) if isinstance(mx,(int,float)) and math.isfinite(mx) and mx>0 else None
    result={
      'schema':'EXP073JL_ARTICLE3_LAYERB_COMMON_GRID_THIRD_REFINEMENT_CONVERGENCE_RESULT_V0_1',
      'experiment':'Exp073JL',
      ...
      'inherited_slot_remapping':{'10':'coarse_guarded_C2048_2049_nodes','20':'fine_guarded_F4096_4097_nodes'},
      'ji_geometry_stream_sha256':JI_GEOM_SHA,
      'lattices':{
        'coarse':{'base_n':2048,'ratio_binary64':float(r2048),'lower_guard_count':l2048,'upper_guard_count':u2048,'requested_node_count':len(coarse),'extended_k_min':float(coarse[0]),'extended_k_max':float(coarse[-1])},
        'fine':{'base_n':4096,'ratio_binary64':float(r4096),'lower_guard_count':l4096,'upper_guard_count':u4096,'requested_node_count':len(fine),'extended_k_min':float(fine[0]),'extended_k_max':float(fine[-1])}
      },
      ...
      'token':'PASS_EXP073JL_COMMON_GRID_THIRD_REFINEMENT_CONVERGED_V0_1' if converged else 'PASS_EXP073JL_COMMON_GRID_THIRD_REFINEMENT_NOT_CONVERGED_V0_1'
    }
