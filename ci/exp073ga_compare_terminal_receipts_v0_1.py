...
def main():
 ...
 s=BASE.read_text()
 for old,new in [('exp073fm','exp073ga'),('EXP073FM','EXP073GA'),('ww_s1_s1','ww_s3_s3'),('ww-s1-s1','ww-s3-s3'),('WW_S1_S1','WW_S3_S3'),('S1->S1','S3->S3'),('[1,1]','[3,3]')]:
  if old not in s: raise RuntimeError(f'fail-closed missing GA comparator token {old!r}')
  s=s.replace(old,new)
 for t in ("'source_pair':'S3->S3'","'ordered_source_indices':[3,3]","'same_field_object_handoff':True",'checkpoints/exp073ga-ww-s3-s3-a-v0-1','checkpoints/exp073ga-ww-s3-s3-b-v0-1','PASS_EXP073GA_WW_S3_S3_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'):
  if t not in s: raise RuntimeError(f'fail-closed missing GA comparator invariant {t!r}')
 for t in ("'source_pair':'S1->S1'","'ordered_source_indices':[1,1]",'checkpoints/exp073ga-ww-s1-s1-a-v0-1','checkpoints/exp073ga-ww-s1-s1-b-v0-1','PASS_EXP073FM_WW_S1_S1_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'):
