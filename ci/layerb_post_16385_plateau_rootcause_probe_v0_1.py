...
def common(a):
 ...
 if len(n8)!=8193 or len(n16)!=16385 or not np.array_equal(n8,n16[::2]): raise RuntimeError('canonical nesting mismatch')
...
def numeric(a,lane):
 jo,jj,rp,n8,n16,t=common(a)
 r8,rec8,l8,u8,m8=run_grid(jo,jj,n8,t,a,9216)
 r16,rec16,l16,u16,m16=run_grid(jo,jj,n16,t,a,18432)
 base={'effect':'+0/+0','selection_rule':'canonical_8193_indices_[1024,2048,4096,6144,7168]','selected_indices_8193':QIDX.tolist(),'selected_indices_16385':(2*QIDX).tolist(),'selected_k_binary64_hex':[float(x).hex() for x in t],'z_anchors_binary64_hex':[float(z).hex() for z in ZANCH],'h':H,'coarse_node_sha256':jo.NODE_SHA,'fine_node_sha256':rp.NODE_SHA,'lifecycle_8193':l8,'lifecycle_16385':l16,'unsupported_target_evaluations':u8+u16,'max_requested_node_coordinate_rel_mismatch':max(m8,m16),'full_107_row_traversal_executed':False,'scientific_authority_created':False,'next_rung_authorized':False,'covariance_restriction_authorized':False,'Wm_S3_opened':False}
 ...
 for label,raw in [('8193',r8),('16385',r16)]:
  ...
  for z in ZANCH:
   ...
   na=am-ref; nb=bp-bm; ra=np.abs(na/(-H)); rb=np.abs(nb/(2*H)); sa=np.maximum(np.maximum(np.abs(am),np.abs(ref)),np.finfo(float).tiny); sb=np.maximum(np.maximum(np.abs(bp),np.abs(bm)),np.finfo(float).tiny)
   ca=np.abs(na)/sa; cb=np.abs(nb)/sb; aa=1.0/np.maximum(ca,np.finfo(float).tiny); ab=1.0/np.maximum(cb,np.finfo(float).tiny); maxamp=max(maxamp,float(max(np.max(aa),np.max(ab))))
...
def interpolation(a):
 jo,jj,rp,n8,n16,t=common(a)
 funcs={'log_cubic':lambda x:1.0+0.2*np.log(x)-0.03*np.log(x)**2+0.004*np.log(x)**3,'smooth':lambda x:np.sin(0.7*np.log(x))+0.1*np.cos(1.3*np.log(x))}
 shared=np.array_equal(n8,n16[::2]); controls={}; mx=0.0
 for name,f in funcs.items():
  ...
  e8=np.abs(v8-truth); e16=np.abs(v16-truth); cross=np.abs(v8-v16); mx=max(mx,float(np.max(cross)))
  controls[name]={'8193_abs_residual':e8.tolist(),'16385_abs_residual':e16.tolist(),'cross_grid_abs_difference':cross.tolist(),'max_8193_abs_residual':float(np.max(e8)),'max_16385_abs_residual':float(np.max(e16)),'max_cross_grid_abs_difference':float(np.max(cross))}
 return {'schema':'LAYERB_POST_16385_CANONICAL_INTERPOLATION_CONTROL_V0_1','classification':INT_PASS,'effect':'+0/+0','exact_shared_node_nesting':shared,'shared_node_count':len(n8),'selection_rule':'canonical_8193_indices_[1024,2048,4096,6144,7168]','selected_indices_8193':QIDX.tolist(),'selected_indices_16385':(2*QIDX).tolist(),'selected_k_binary64_hex':[float(x).hex() for x in t],'controls':controls,'max_cross_grid_control_difference':mx,'science_result_read':False,'class_solver_invoked':False,'scientific_authority_created':False,'next_rung_authorized':False,'covariance_restriction_authorized':False,'Wm_S3_opened':False,'token':INT_PASS}
