...
PASS='PASS_EXP073AJ_AI_ENVIRONMENT_PROVENANCE_CLASSIFIER_SYNTHETIC_V0_1'
...
def classify(a,b):
    ...
    if a['thread_env']!=THREAD or b['thread_env']!=THREAD: label='CONTROL_DRIFT'
    elif a['versions_and_numpy_config']!=b['versions_and_numpy_config']: label='SOFTWARE_BUILD_DRIFT'
    else:
        diff=[]
        for k in PLATFORM:
    ...
    return {'experiment':'Exp073AJ','label':label,'readiness_increment':0,'article3_scientific_readiness_percent':52,'gate_state':GATES,'science_gate_scored':False,'production_release':False,'numerical_result_read':False,'resource_sha256':{r:{k:h(str(r[k])) for k in ['memory','filesystem','ulimit']} for r in [a,b]}}
...
def selftest():
    ...
    try: classify(a,bad); raise AssertionError('accepted bad')
...
def main():
    ...
    if x.self_test:
        n=selftest(); out={'experiment':'Exp073AJ','status':PASS,'synthetic_only':True,'tests_passed':n,'readiness_increment':0,'article3_scientific_readiness_percent':52,'gate_state':GATES,'science_gate_scored':False,'production_release':False,'real_ai_receipts_read':False}
    else: out=classify(json.load(open(x.a)),json.load(open(x.b)))
