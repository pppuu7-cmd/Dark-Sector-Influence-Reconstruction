...
def main():
 ...
 for x in ['driver','prereg','fresh_helper','adapter','expected_driver_blob','expected_prereg_blob','expected_fresh_blob','expected_adapter_blob','source_head','out']: ap.add_argument('--'+x.replace('_','-'),required=True)
 a=ap.parse_args(); p=Path(a.driver); src=p.read_text(); tree=ast.parse(src); funcs={n.name:n for n in tree.body if isinstance(n,ast.FunctionDef)}; run=funcs['run_replica']; cmpf=funcs['compare_replicas']; maskf=funcs['fresh_or_restore_masks']; run_src=ast.unparse(run); cmp_src=ast.unparse(cmpf)
 run_calls=[call_name(c) for c in ast.walk(run) if isinstance(c,ast.Call)]; mask_calls=[call_name(c) for c in ast.walk(maskf) if isinstance(c,ast.Call)]
 provenance={'driver_blob':blob_sha(p)==a.expected_driver_blob,'prereg_blob':blob_sha(Path(a.prereg))==a.expected_prereg_blob,'fresh_helper_blob':blob_sha(Path(a.fresh_helper))==a.expected_fresh_blob,'adapter_blob':blob_sha(Path(a.adapter))==a.expected_adapter_blob}
 compact=src.replace(' ',''); run_compact=run_src.replace(' ','')
 checks={
 'exact_edges':repr(EDGES).replace(' ','') in compact,
 ...
 'same_fields_feed_workspace':'w.compute_coupling_matrix(f0,f2,b)' in run_compact,
 'same_fields_feed_pcl':'f0.get_mask_alms()' in run_src and 'f2.get_mask_alms()' in run_src,
 'stock_write_to':'.write_to(str(workspace_path))' in run_src,
 ...
 'exact_sha_comparator':"a['selected_te_sha256']==b['selected_te_sha256']" in cmp_src.replace(' ',''),
 'numpy_array_equal':'np.array_equal' in cmp_src,
 'no_tolerance_rescue':all(x not in src for x in ['allclose','isclose','rtol','atol']),
 'worker_contract':'OUTER_COMPUTE_WORKERS=8' in compact,
 'nested_threads':all((repr(k)+':'+repr(v)) in compact for k,v in THREADS.items()),
 'failclosed_identity':'fail-closed checkpoint identity mismatch' in src,
 'resume_final_before_expensive':run_src.index('validated_finished_receipt(')<run_src.index("load_manifest(root, 'fresh_workspace_mcm_complete'")<run_src.index('fresh_or_restore_masks('),
 'source_head_binding':'source_head' in src and 'contract_fingerprint' in src,
 'historical_import_false':"'historical_wm_s3_numerical_import':False" in compact,
 'cross_replica_false':"'other_replica_output_read':False" in compact}
 ...
 out={'schema':'dsir.exp073cz.wm_s3.unified_production_ab_driver_audit.v0.2','status':status,'accounting':'+0/+0','science_gate_scored':False,'wm_s3_authority_created':False,'exp073bu_activated':False,'source_head':a.source_head,'provenance':provenance,'checks':checks,'frozen_edges':EDGES,'checkpoint_order':ORDER,'namespaces':NAMESPACES,'no_des_scale_numerics_executed':True,'verifier_scope':'run_replica AST for resume-order check'}
