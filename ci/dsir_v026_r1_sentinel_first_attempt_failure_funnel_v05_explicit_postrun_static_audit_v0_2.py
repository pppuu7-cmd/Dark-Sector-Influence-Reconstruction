...
def main():
    ...
    require(t,'name: dsir-v026-r1-first-attempt-failure-funnel-hosted-v0-5','workflow_dispatch:','test "$GITHUB_RUN_NUMBER" = "1"','test "$GITHUB_RUN_ATTEMPT" = "1"',"int(only['run_number'])==1","int(only['run_attempt'])==1",'Fresh end-of-run target history closure',"successor_science_authorized_by_this_receipt",'full_107_row_execution_authorized')
    ...
    require(o,'name: dsir-v026-r1-first-attempt-failure-funnel-orchestrator-v0-5','workflow_dispatch:','actions: write','Atomic zero-history preflight','Dispatch exactly one v0.5 target','Wait for unique target to become terminal success','Dispatch terminal-history auditor only after target terminal success','Wait for unique terminal-history auditor success',"target['status']=='completed' and target['conclusion']=='success'",'postrun_dispatch_after_target_terminal',"workflow_id=os.path.basename(os.environ['TARGET_WORKFLOW_PATH'])", "workflow_id=os.path.basename(os.environ['TERMINAL_WORKFLOW_PATH'])")
    ...
    out={
      'schema':'LAYERB_BETA_V0_26_R1_SENTINEL_FIRST_ATTEMPT_FAILURE_FUNNEL_V0_5_EXPLICIT_POSTRUN_STATIC_AUDIT_V0_2',
      ...
      'filename_workflow_dispatch_ids_verified':True,'orchestrator_waits_for_target_terminal_before_postrun_dispatch':True,
      ...
      'token':'QUALIFIED_LAYERB_BETA_V0_26_R1_SENTINEL_FIRST_ATTEMPT_FAILURE_FUNNEL_V0_5_EXPLICIT_POSTRUN_STATIC_V0_2_PLUS_0_PLUS_0'
    }
