...
def main():
 ...
 for rep in ('A','B'):
  ...
  assert rec['source_pair']=='S2->S2' and rec['ordered_source_indices']==[2,2] and rec['same_field_object_handoff'] is True and rec['historical_ww_numerical_import'] is False and rec['other_replica_output_read'] is False
  assert src['ordered_source_indices']==[2,2] and src['reconstruction_counts']=={'s2':1} and src['same_source_map_both_sides'] is True
  ids=ws['field_object_ids']; assert ws['field_construction_count']==1 and ws['same_field_object_handoff'] is True and len(ids)==2 and ids[0]==ids[1]
  ...
  assert prune['complete_chain_verified_before_prune'] is True and prune['preserved_complete_receipt'] is True and prune['pruned_only_after_receipt'] is True and prune['same_field_object_handoff'] is True and set(prune['stage_manifest_sha256'])==set(STAGES)
  for st in STAGES:
 ...
 print('PASS_EXP073FX_WW_S2_S2_FILEBACKED_PROVENANCE_ADMISSION_V0_1')
 print('classification=SCIENTIFIC_AUTHORITY_ADMITTED')
 print('ww_s2_s2_authority_created=true')
