...
PASS='PASS_EXP073AK2_AI_COMPLETION_AUTHORITY_CLASSIFIER_SYNTHETIC_V0_2'
...
def classify(d):
    ...
    if not out:
        ...
        out='PENDING_EXP073AI' if any_active else 'INVALID_CONTROL_PLANE_STATE_NO_SCIENCE_CLASSIFICATION'
    return {'experiment':'Exp073AK2','classification':out,'preserved_exp073ai_token':preserved,
...
def selftest():
    ...
    d=fixture(); d['replica_a']={'state':'completed','conclusion':'failure','artifact_complete':False}; check('replica_failure',d,'INCOMPLETE_INFRASTRUCTURE_EXP073AI_REPLICA_EXECUTION')
    d=fixture(); d['replica_a']={'state':'completed','conclusion':'success','artifact_complete':False}; check('replica_success_missing_artifact',d,'INCOMPLETE_INFRASTRUCTURE_EXP073AI_REPLICA_ARTIFACT')
    for token in sorted(VALID):
        ...
        check('valid_'+token[:8],d,'VALID_HOSTED_EXP073AI_CLASSIFICATION',token)
...
def main():
    ...
    if a.self_test:
        t=selftest(); out={'experiment':'Exp073AK2','status':PASS,'synthetic_only':True,'tests_passed':len(t),'tests':t,'real_ai_receipt_read':False,'article3_scientific_readiness_percent':52,'readiness_increment':0,'gate_state':GATES,'science_gate_scored':False,'production_release':False}
