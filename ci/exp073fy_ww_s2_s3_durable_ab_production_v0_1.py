...
def transform_source():
 ...
 for old,new in [('exp073fs','exp073fy'),('EXP073FS','EXP073FY'),('ww_s1_s2','ww_s2_s3'),('ww-s1-s2','ww-s2-s3'),('WW_S1_S2','WW_S2_S3'),('S1S2','S2S3'),('S1->S2','S2->S3'),('[1,2]','[2,3]'),('load_s1s2','load_s2s3')]:
  ...
  s=s.replace(old,new)
 # Protect already-correct hyphenated checkpoint namespaces before the generic
 # symbol shift. Otherwise regex s2->s3 mutates S2-S3 identity to S3-S3.
 namespace_protected=[
  ('checkpoints/exp073fy-ww-s2-s3-a-v0-1','__FY_NAMESPACE_A__'),
  ('checkpoints/exp073fy-ww-s2-s3-b-v0-1','__FY_NAMESPACE_B__')]
 for old,new in namespace_protected:
  if old not in s: raise RuntimeError(f'fail-closed missing FY namespace token {old!r}')
  s=s.replace(old,new)
 ...
 restore={
  ...
  '__FY_NAMESPACE_A__':'checkpoints/exp073fy-ww-s2-s3-a-v0-1','__FY_NAMESPACE_B__':'checkpoints/exp073fy-ww-s2-s3-b-v0-1'}
 for old,new in restore.items(): s=s.replace(old,new)
 required=["source_count_map(r1_root,2)","source_count_map(r1_root,3)","'ordered_source_indices':[2,3]","'source_pair':'S2->S3'","compute_coupling_matrix(f2,f3,b)","field_construction_count':2","same_field_object_handoff':False","reconstruction_counts':{'s2':1,'s3':1}","'A':'checkpoints/exp073fy-ww-s2-s3-a-v0-1'","'B':'checkpoints/exp073fy-ww-s2-s3-b-v0-1'","PASS_EXP073FY_WW_S2_S3_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1"]
 for t in required:
  if t not in s: raise RuntimeError(f'fail-closed missing FY invariant {t!r}')
 for t in ("'ordered_source_indices':[1,2]","'source_pair':'S1->S2'",'PASS_EXP073FS_WW_S1_S2_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1','exp073fy-ww-s3-s3-'):
  if t in s: raise RuntimeError(f'fail-closed stale/invalid FY token {t!r}')
