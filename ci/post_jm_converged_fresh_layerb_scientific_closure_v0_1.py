...
JM_CONVERGED = "COMMON_GRID_FOURTH_REFINEMENT_CONVERGED_PLUS_0_PLUS_0"
SCI_PASS = "PASS_PHYSICAL_SUPPORT_ARTICLE3_SHARED_GRID_POST_JM_V0_1"
SCI_FAIL = "FAIL_PHYSICAL_SUPPORT_ARTICLE3_SHARED_GRID_POST_JM_V0_1"
NUM_UNRES = "NUMERICALLY_UNRESOLVED_POST_JM_LAYERB_CLOSURE_V0_1"
...
CANONICAL_8193_AUTH_CLASS = "CANONICAL_8193_ANCHOR_SELECTED_QUORUM_PASS_PLUS_0_PLUS_0"
...
def validate_activation(path: str | Path) -> dict:
    ...
    if jm.get("classification") != JM_CONVERGED:
        raise RuntimeError("post-JM scientific closure activation condition not met")
    if jm.get("artifact_verified_independently") is not True:
        raise RuntimeError("activating JM artifact is not independently verified")
    if not is_sha256(jm.get("artifact_zip_sha256")) or not is_sha256(jm.get("result_json_sha256")):
        raise RuntimeError("activating JM provenance digest missing")
    return jm


def validate_canonical_8193(path: str | Path) -> dict:
    d = json.loads(Path(path).read_text())
    if (
        d.get("classification") != CANONICAL_8193_AUTH_CLASS
        or d.get("artifact_verified_independently") is not True
        or d.get("branch_activated") is not False
        or d.get("scientific_response_read") is not False
        or d.get("class_solver_invoked") is not False
        or d.get("requested_node_count") != 8193
        or d.get("guard_counts") != [0, 1]
        or d.get("node_payload_sha256") != FINE_NODE_SHA
        or d.get("u64hex_sha256") != FINE_TEXT_SHA
        or d.get("anchor_4097_node_sha256") != COARSE_NODE_SHA
        or d.get("anchor_4097_u64hex_sha256") != COARSE_TEXT_SHA
        or d.get("anchor_match_count", 0) < d.get("minimum_anchor_quorum", 999)
    ):
        raise RuntimeError("invalid canonical 8193 authority")
    return d
...
def validate_capacity_patch(path: str | Path) -> dict:
    ...
    if (
        ...
        raise RuntimeError("invalid post-JM closure capacity patch")
    return d


def execute(a) -> dict:
    jm = validate_activation(a.jm_authority)
    canon8193 = validate_canonical_8193(a.canonical_8193_authority)
    ...
    coarse = engine.load_nodes(a.coarse, 4097, COARSE_TEXT_SHA, COARSE_NODE_SHA)
    fine = engine.load_nodes(a.fine, 8193, FINE_TEXT_SHA, FINE_NODE_SHA)
    ...
    return {
        "schema": "POST_JM_FRESH_LAYERB_SCIENTIFIC_CLOSURE_RESULT_V0_1",
        ...
        "activation_authority": {
            "jm_classification": jm.get("classification"),
            "jm_run_id": jm.get("run_id"),
            "jm_job_id": jm.get("job_id"),
            "jm_workflow_head_sha": jm.get("workflow_head_sha"),
            "jm_artifact_id": jm.get("artifact_id"),
            "jm_artifact_zip_sha256": jm.get("artifact_zip_sha256"),
            "jm_result_json_sha256": jm.get("result_json_sha256"),
            "response_values_reused": False,
        },
        "canonical_lattices": {
            "coarse": {"requested_node_count": 4097, "text_sha256": COARSE_TEXT_SHA, "node_sha256": COARSE_NODE_SHA},
            "fine": {"requested_node_count": 8193, "text_sha256": FINE_TEXT_SHA, "node_sha256": FINE_NODE_SHA},
        },
        "canonical_8193_authority": {
        ...
        "token": (
            "CANDIDATE_POST_JM_LAYERB_SCIENTIFIC_PASS_REQUIRES_INDEPENDENT_VERIFICATION"
...
def main() -> int:
    ...
    ap.add_argument("--canonical-8193-authority", required=True)
