...
for old,new in [('exp073fm','exp073ga'),('Exp073FM','Exp073GA'),('EXP073FM','EXP073GA'),('ww_s1_s1','ww_s3_s3'),('ww-s1-s1','ww-s3-s3'),('WW_S1_S1','WW_S3_S3'),('S1->S1','S3->S3'),('[1,1]','[3,3]')]:
    if old not in s: raise SystemExit(f'fail-closed missing GA home transform token {old!r}')
    s=s.replace(old,new)
required=['ci/exp073ga_ww_s3_s3_durable_ab_production_v0_1.py','ci/exp073ga_ww_s3_s3_durable_ab_production_v0_2.py','ci/exp073ga_verify_and_prune_replica_v0_1.py','ci/exp073ga_compare_terminal_receipts_v0_1.py','exp073ga-ww-s3-s3-filebacked-ab-v0-1',"'source_pair':'S3->S3'","'ordered_source_indices':[3,3]"]
for t in required:
    if t not in s: raise SystemExit(f'fail-closed missing GA home invariant {t!r}')
