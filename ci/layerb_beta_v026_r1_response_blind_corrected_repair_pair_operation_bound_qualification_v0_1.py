#!/usr/bin/env python3
"""Response-blind corrected repair-pair qualification with exact operation-parameter binding."""
...
PARENT_TERMINAL = ROOT / "docs/dsir4/authority/LAYERB_BETA_V0_26_R1_RESPONSE_BLIND_CORRECTED_REPAIR_PAIR_OPERATIONAL_QUALIFICATION_TERMINAL_V0_1.json"
PARENT_RUNTIME_CRITIC = ROOT / "docs/dsir4/authority/LAYERB_BETA_V0_26_R1_RESPONSE_BLIND_CORRECTED_REPAIR_PAIR_OPERATIONAL_QUALIFICATION_RUNTIME_CRITIC_V0_1.json"
...
PARENT_TERMINAL_BLOB = "c851c8c93c1228ae268808dd0ef707fe190a02ae"
PARENT_RUNTIME_CRITIC_CLASSIFICATION = "INVALID_IMPLEMENTATION_NEGATIVE_CONTROL_ALIASING_CONFIRMED"
...
def validate_frozen_dependencies() -> dict:
    ...
    require_blob(PARENT_TERMINAL, PARENT_TERMINAL_BLOB, "parent_terminal_binding")
    ...
    if corrected.get("payload_sha256") != PAYLOAD_SHA256 or corrected.get("negative_control_sha256") != NEGATIVE_PAYLOAD_SHA256:
    ...
    parent_terminal = json.loads(PARENT_TERMINAL.read_text(encoding="utf-8"))
    if parent_terminal.get("classification") != "INVALID_IMPLEMENTATION":
        pair_fail("INVALID_IMPLEMENTATION", "parent_terminal_binding", "parent qualification closure mismatch")
    runtime_critic = json.loads(PARENT_RUNTIME_CRITIC.read_text(encoding="utf-8"))
    if runtime_critic.get("classification") != PARENT_RUNTIME_CRITIC_CLASSIFICATION:
        pair_fail("INVALID_IMPLEMENTATION", "parent_terminal_binding", "parent runtime Critic closure mismatch")
    return terminal


def validate_historical(raw: bytes) -> list[bytes]:
    if len(raw) != V01_LEN or sha256(raw) != V01_SHA256 or git_blob(raw) != V01_BLOB:
        pair_fail("REPAIR_IDENTITY_FAIL", "historical_source", "historical V0.1 identity mismatch")
    if not raw.endswith(b"\n"):
        pair_fail("REPAIR_IDENTITY_FAIL", "historical_source", "historical V0.1 final LF mismatch")
    lines = raw.splitlines()
    if len(lines) != LINE_COUNT or lines[775] != b"3f9c8a9eea3c9c8":
        pair_fail("REPAIR_IDENTITY_FAIL", "historical_source", "historical V0.1 structure/token mismatch")
    return lines


def validate_repair_operation(offset: int, insert_byte: bytes) -> None:
    if offset != INSERT_OFFSET:
        pair_fail("REPAIR_IDENTITY_FAIL", "repair_operation", f"repair offset mismatch: {offset} != {INSERT_OFFSET}")
    if insert_byte != INSERT_BYTE:
        pair_fail("REPAIR_IDENTITY_FAIL", "repair_operation", "repair inserted byte mismatch")
    if len(insert_byte) != 1:
        pair_fail("INVALID_IMPLEMENTATION", "repair_operation", "repair must insert exactly one byte")


def reconstruct(raw: bytes, offset: int = INSERT_OFFSET, insert_byte: bytes = INSERT_BYTE) -> bytes:
    validate_repair_operation(offset, insert_byte)
...
def validate_corrected(candidate: bytes, expected_sha: str = CORRECTED_SHA256, expected_blob: str = CORRECTED_BLOB) -> tuple[list[bytes], bytes]:
    ...
    if len(payload) != PAYLOAD_LEN or sha256(payload) != PAYLOAD_SHA256:
        pair_fail("REPAIR_IDENTITY_FAIL", "payload_identity", "payload identity mismatch")
...
def run_repair_negative_controls(raw: bytes, corrected: bytes, payload: bytes) -> dict[str, bool]:
    ...
    return {
        ...
        "wrong_insert_byte_operation_rejected": expect_repair_rejection(
            "wrong insert byte operation", lambda: reconstruct(raw, INSERT_OFFSET, wrong_byte)
        ),
        "wrong_insert_offset_operation_rejected": expect_repair_rejection(
            "wrong insert offset operation", lambda: reconstruct(raw, INSERT_OFFSET + 1, INSERT_BYTE)
        ),
...
def main() -> int:
    ...
    result = {
        "schema": "LAYERB_BETA_V0_26_R1_RESPONSE_BLIND_CORRECTED_REPAIR_PAIR_OPERATION_BOUND_QUALIFICATION_V0_1",
    ...
    try:
        transport_terminal = validate_frozen_dependencies()
        raw = CANONICAL_V01.read_bytes()
        historical_lines = validate_historical(raw)
        validate_repair_operation(INSERT_OFFSET, INSERT_BYTE)
        ...
        if f"sha256:{sha256(zip_bytes)}" != transport.ARTIFACT_DIGEST or sha256(receipt_bytes) != transport.MEMBER_SHA256:
            pair_fail("PROVENANCE_FAIL", "joint_provenance", "artifact/member digest mismatch")
        ...
        result.update(
            {
                ...
                "repair_operation": {
                    "operation_parameters_validated_before_reconstruction": True,
