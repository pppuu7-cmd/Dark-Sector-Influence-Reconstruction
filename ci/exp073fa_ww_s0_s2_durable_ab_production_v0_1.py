...
SCHEMA='dsir.exp073fa.ww_s0_s2.durable_ab_production.v0.1'
...
def load_manifest(root,stage,replica,head,fp):
 ...
 if not ok: raise RuntimeError('fail-closed WW_S0_S2 checkpoint identity mismatch')
 return r

def fresh_or_restore_sources(root,replica,r1_root,r1_digest,head,fp):
 st=load_manifest(root,'fresh_sources_complete',replica,head,fp); p0=root/'s0_count_map.npy'; p2=root/'s1_count_map.npy'
 if st is None:
  auth=validate_r1(r1_root,r1_digest); s0,m0=source_count_map(r1_root,0); h0=atomic_npy(p0,s0); del s0; gc.collect(); s2,m1=source_count_map(r1_root,2); h1=atomic_npy(p2,s2); del s2; gc.collect()
  st=manifest(root,'fresh_sources_complete',replica,head,fp,{'s0_count_map':{'canonical_sha256':h0,'shape':[12*4096*4096],'dtype':'<f8'},'s1_count_map':{'canonical_sha256':h1,'shape':[12*4096*4096],'dtype':'<f8'},'r1_authority':auth,'s0_authority':m0,'s1_authority':m1,'ordered_source_indices':[0,2],'reconstruction_counts':{'s0':1,'s2':1}})
 for p,k in ((p0,'s0_count_map'),(p2,'s1_count_map')):
...
def validated_finished(root,replica,head,fp):
 ...
 if r.get('ordered_source_indices')!=[0,2] or r.get('same_field_object_handoff') is not False or r.get('bpw_route')!='public_get_bandpower_windows_after_filebacked_fits_read': raise RuntimeError('fail-closed ordered public-route receipt mismatch')
 return r
...
def run_replica(replica,args):
 ...
 if ws is None:
  p0,p2,src=fresh_or_restore_sources(root,replica,Path(args.r1_root),args.r1_artifact_digest,args.source_head,args.contract_fingerprint)
  s0=np.load(p0,mmap_mode='r',allow_pickle=False); f0=nmt.NmtField(s0,None,spin=2); del s0; gc.collect()
  s2=np.load(p2,mmap_mode='r',allow_pickle=False); f2=nmt.NmtField(s2,None,spin=2); del s2; gc.collect()
  if id(f0)==id(f2): raise RuntimeError('fail-closed distinct-field identity collision')
  ids=[id(f0),id(f2)]; b=nmt.NmtBin.from_edges(BAND_EDGES[:-1],BAND_EDGES[1:]); w=nmt.NmtWorkspace(); w.compute_coupling_matrix(f0,f2,b); w.write_to(str(wp)); wsha=file_sha(wp)
  ws=manifest(root,'fresh_workspace_mcm_complete',replica,args.source_head,args.contract_fingerprint,{'workspace_fits':{'sha256':wsha},'same_field_object_handoff':False,'ordered_source_indices':[0,2],'field_object_ids':ids,'source_map_sha256':[src['payloads']['s0_count_map']['canonical_sha256'],src['payloads']['s1_count_map']['canonical_sha256']]})
  del w,b,f2,f0; gc.collect()
 else:
  if ws['payloads'].get('same_field_object_handoff') is not False or ws['payloads'].get('ordered_source_indices')!=[0,2]: raise RuntimeError('fail-closed restored field-order semantics mismatch')
 ...
 manifest(root,'mcm_fits_verified',replica,args.source_head,args.contract_fingerprint,{'workspace_fits':{'sha256':wsha},'ordered_source_indices':[0,2],'same_field_object_handoff':False})
...
def compare(a,b,out):
 for r in (a,b):
  if r.get('source_pair')!='S0->S2' or r.get('ordered_source_indices')!=[0,2] or r.get('same_field_object_handoff') is not False or r.get('bpw_route')!='public_get_bandpower_windows_after_filebacked_fits_read': raise RuntimeError('fail-closed A/B semantics')
 ...
 status='PASS_EXP073FA_WW_S0_S2_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1' if se and ae and finite else 'FAIL_EXP073FA_WW_S0_S2_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'; r={'schema':SCHEMA+'.ab_compare','status':status,'sha256_equal':se,'numpy_array_equal':ae,'all_finite':finite,'a_sha256':a['selected_ee_sha256'],'b_sha256':b['selected_ee_sha256'],'source_pair':'S0->S2','ordered_source_indices':[0,2],'bpw_route':'public_get_bandpower_windows_after_filebacked_fits_read','same_field_object_handoff':False,'no_tolerance_rescue':True,'science_gate_scored':True,'ww_s0_s2_authority_created':False}; atomic_json(out,r); return r
