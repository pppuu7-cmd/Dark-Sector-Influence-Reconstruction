...
def interp_record(k,y,t):
    ...
    if lo==hi: frac=0.0; val=float(y[lo])
    else:
...
def main():
    ...
    result={'schema':'EXP073JC_ARTICLE3_LAYERB_REFERENCE_BRACKET_ORIGINAL_PAIR_RESULT_V0_1','experiment':'Exp073JC','classification':'INVALID_INFRA_PLUS_0_PLUS_0','h':H,'k_per_decade_ladder':list(KPDS),'probes':{},'scientific_authority_created':False,'covariance_restriction_authorized':False,'effect':'+0/+0','historical_rel_tol_reference':1e-3}
    try:
        for kpd in KPDS:
            cs={}
            try:
                ...
                for component,z,target in PROBES:
                    ...
                    transported={}
                    for name,_,_ in MODELS:
                        k,y=tables[name]; vlo=interp_value(k,y,klo); vhi=interp_value(k,y,khi); vtar=float(vlo+frac*(vhi-vlo))
        ...
        for component,_,_ in PROBES:
            ...
            rec['native_10_to_20_relative_difference']=rel(n[10.0],n[20.0])
            rec['common_10_to_20_relative_difference']=rel(c[10.0],c[20.0])
        result['classification']='ORIGINAL_PAIR_REFERENCE_BRACKET_OBSERVED_PLUS_0_PLUS_0'; result['token']='PASS_EXP073JC_ORIGINAL_PAIR_REFERENCE_BRACKET_OBSERVED_V0_1'
    except Exception as e:
        result['error']=repr(e); result['token']='INVALID_EXP073JC_ORIGINAL_PAIR_REFERENCE_BRACKET_V0_1'
    ...
    raise SystemExit(0 if result['classification']=='ORIGINAL_PAIR_REFERENCE_BRACKET_OBSERVED_PLUS_0_PLUS_0' else 2)
