...
PASS='PASS_EXP073FM_WW_S1_S1_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'
FAIL='FAIL_EXP073FM_WW_S1_S1_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'
...
def load_receipt(root:Path,replica:str):
    rr=root/'checkpoints'/replica; rp=rr/'replica_receipt.json'; ep=rr/'exact_route'/'selected_ee.bin'; cp=rr/'replica_receipt_complete.json'; pp=rr/'post_receipt_prune.json'
    ...
    required={'replica':replica,'source_pair':'S1->S1','ordered_source_indices':[1,1],'same_field_object_handoff':True,'bpw_route':'public_get_bandpower_windows_after_filebacked_fits_read','source_head':FROZEN_SOURCE_HEAD,'contract_fingerprint':FROZEN_CONTRACT,'checkpoint_namespace':NAMESPACES[replica],'historical_ww_numerical_import':False,'other_replica_output_read':False,'science_gate_scored':False}
    for k,v in required.items():
        if r.get(k)!=v: raise RuntimeError(f'fail-closed receipt identity mismatch {replica}:{k}')
    preq={'replica':replica,'checkpoint_namespace':NAMESPACES[replica],'source_head':FROZEN_SOURCE_HEAD,'contract_fingerprint':FROZEN_CONTRACT,'source_pair':'S1->S1','ordered_source_indices':[1,1],'same_field_object_handoff':True,'complete_chain_verified_before_prune':True,'preserved_complete_receipt':True,'pruned_only_after_receipt':True,'no_tolerance_rescue':True}
