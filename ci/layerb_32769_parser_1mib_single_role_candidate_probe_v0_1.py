...
NATIVE_KPD = 20.0
CAPACITY = 32769
OLD_PARSER_CAPACITY = 524288
CANDIDATE_PARSER_CAPACITY = 1048576
NODE_COUNT = 32769
...
BUILD_PASS = "LAYERB_32769_CLASS_CAPACITY_BUILD_ENVELOPE_PASS_PLUS_0_PLUS_0"
EXPECTED_ROLES = ("reference", "alpha_minus", "beta_plus", "beta_minus")
PASS = "LAYERB_32769_PARSER_1MIB_SINGLE_ROLE_CANDIDATE_PASS_PLUS_0_PLUS_0"
FAIL = "LAYERB_32769_PARSER_1MIB_SINGLE_ROLE_CANDIDATE_FAIL_PLUS_0_PLUS_0"
...
def main() -> int:
    ...
    try:
        ...
        if ba.get("classification") != BUILD_PASS or ba.get("artifact_verified_independently") is not True or ba.get("class_commit") != "ac627d54e9ce196a08878d1ba33999819925d19c" or ba.get("new_capacity") != CAPACITY:
            raise RuntimeError("capacity build authority invalid")
        if ba.get("parser_new_argument_capacity") != OLD_PARSER_CAPACITY:
            raise RuntimeError("expected superseded parser baseline mismatch")
        ...
        if payload_bytes > CANDIDATE_PARSER_CAPACITY:
            raise RuntimeError(f"candidate parser too small: {payload_bytes}>{CANDIDATE_PARSER_CAPACITY}")
        ...
        result = {
            "schema": "LAYERB_32769_PARSER_1MIB_SINGLE_ROLE_CANDIDATE_RESULT_V0_1",
            "classification": PASS,
            "effect": "+0/+0",
            "role": role,
            ...
            "capacity": CAPACITY,
            "superseded_parser_capacity": OLD_PARSER_CAPACITY,
            "candidate_parser_capacity": CANDIDATE_PARSER_CAPACITY,
            "k_output_values_bytes_including_nul": payload_bytes,
            "candidate_parser_margin_bytes": CANDIDATE_PARSER_CAPACITY - payload_bytes,
