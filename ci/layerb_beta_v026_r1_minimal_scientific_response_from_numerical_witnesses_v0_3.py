...
TARGET_ID_CORRECTION = ROOT / "docs/dsir4/authority/LAYERB_BETA_V0_26_R1_MINIMAL_SCIENTIFIC_RESPONSE_TARGET_IDENTITY_PROVENANCE_CORRECTION_V0_1.json"
TARGET_ID_CORRECTION_CRITIC = ROOT / "docs/dsir4/authority/LAYERB_BETA_V0_26_R1_MINIMAL_SCIENTIFIC_RESPONSE_TARGET_IDENTITY_PROVENANCE_CORRECTION_CRITIC_V0_1.json"
IMPLEMENTATION_CRITIC = ROOT / "docs/dsir4/authority/LAYERB_BETA_V0_26_R1_MINIMAL_SCIENTIFIC_RESPONSE_FROM_NUMERICAL_WITNESSES_IMPLEMENTATION_STATIC_CRITIC_V0_3.json"
EXECUTION_AUTHORITY = ROOT / "docs/dsir4/authority/LAYERB_BETA_V0_26_R1_MINIMAL_SCIENTIFIC_RESPONSE_FROM_NUMERICAL_WITNESSES_EXECUTION_AUTHORITY_V0_3.json"
WORKFLOW = ROOT / ".github/workflows/dsir-v026-r1-minimal-scientific-response-from-numerical-witnesses-v0-3.yml"
LAUNCH_MARKER = ROOT / "docs/dsir4/launch/LAYERB_BETA_V0_26_R1_MINIMAL_SCIENTIFIC_RESPONSE_FROM_NUMERICAL_WITNESSES_V0_3.launch.json"
...
TARGET_ID_CORRECTION_BLOB = "27af1bea4c380029741ea56083e982abbbe2fbb1"
TARGET_ID_CORRECTION_CRITIC_BLOB = "7d6f4f9d3b0709ca89993e9a8a46651919f1ffe2"
...
SCI_TOL = 1e-3
TARGET_COUNT = 244
TARGET_ID_SHA256 = "1e835a495e6b742b7b992627e819e949420529ba5097164d260ca0accb0d3c0c"
...
def validate_frozen_chain() -> dict:
    ...
    require_blob(TARGET_ID_CORRECTION, TARGET_ID_CORRECTION_BLOB, "target identity correction")
    require_blob(TARGET_ID_CORRECTION_CRITIC, TARGET_ID_CORRECTION_CRITIC_BLOB, "target identity correction Critic")
    ...
    target_correction = json.loads(TARGET_ID_CORRECTION.read_text(encoding="utf-8"))
    target_correction_critic = json.loads(TARGET_ID_CORRECTION_CRITIC.read_text(encoding="utf-8"))
    ...
    if target_correction.get("status") != "TERMINAL_PROVENANCE_CORRECTION":
        fail("PROVENANCE_FAIL", "authority_binding", "target identity correction status mismatch")
    if target_correction.get("corrected_target_identity_sha256") != TARGET_ID_SHA256:
        fail("PROVENANCE_FAIL", "authority_binding", "corrected target identity mismatch")
    if target_correction_critic.get("verdict") != "PASS_SCOPED":
        fail("PROVENANCE_FAIL", "authority_binding", "target identity correction Critic mismatch")
    return design
...
def api_json(url: str, token: str) -> dict:
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "dsir-minimal-scientific-response-v0-3",
        },
    )
    try:
...
def download_artifact_zip(api: str, repo: str, token: str, artifact_id: int) -> bytes:
    ...
    storage = urllib.request.Request(
        location,
        headers={"Accept": "application/octet-stream", "User-Agent": "dsir-minimal-scientific-response-v0-3"},
    )
