...
def one(root:Path,suffix:str)->Path:
    hits=[p for p in root.rglob('*') if p.is_file() and p.name.endswith(suffix)]
    assert len(hits)==1,(suffix,len(hits),[str(x) for x in hits]); return hits[0]
...
def load(root:Path):
    jp=one(root,'.json')
    # Exclude environment JSON from operator record search.
