...
def main():
    ...
    # v0.3 infrastructure-only repair: preserve the v0.2 hyphenated checkpoint
    # namespace repair and also transform the underscore schema identity.
    # No scientific identity/arithmetic/acceptance changes.
    for old,new in [
    ...
    required=[
        "SCHEMA='dsir.exp073fu.ww_s1_s3.durable_ab_production.v0.1'",
    ...
    for stale in (
        "SCHEMA='dsir.exp073fu.ww_s1_s2.durable_ab_production.v0.1'",
    ...
    mod=types.ModuleType('exp073fu_pruner_v03'); mod.__file__=str(BASE)
    exec(compile(src,'exp073fu_verify_and_prune_replica_v0_3.transformed.py','exec'),mod.__dict__)
    mod.main()
