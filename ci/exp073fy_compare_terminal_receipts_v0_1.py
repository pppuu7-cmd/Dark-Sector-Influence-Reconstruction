...
def main():
 ...
 s=BASE.read_text()
 # Namespace uses hyphens, while authority/science keys use underscores. Transform
 # the exact namespace before the generic experiment-label replacement so the
 # terminal verifier expects the same prospectively frozen FY checkpoint identity.
 ns_old='exp073fs-ww-s1-s2'
 ns_new='exp073fy-ww-s2-s3'
 if ns_old not in s: raise RuntimeError(f'fail-closed missing FY comparator namespace token {ns_old!r}')
 s=s.replace(ns_old,ns_new)
 for old,new in [('exp073fs','exp073fy'),('EXP073FS','EXP073FY'),('ww_s1_s2','ww_s2_s3'),('WW_S1_S2','WW_S2_S3'),('S1->S2','S2->S3'),('[1,2]','[2,3]')]:
  ...
  s=s.replace(old,new)
 for t in ("'A':'checkpoints/exp073fy-ww-s2-s3-a-v0-1'","'B':'checkpoints/exp073fy-ww-s2-s3-b-v0-1'","'source_pair':'S2->S3'","'ordered_source_indices':[2,3]","'same_field_object_handoff':False",'PASS_EXP073FY_WW_S2_S3_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'):
  if t not in s: raise RuntimeError(f'fail-closed missing FY comparator invariant {t!r}')
 for t in ("'source_pair':'S1->S2'","'ordered_source_indices':[1,2]",'exp073fy-ww-s1-s2','PASS_EXP073FS_WW_S1_S2_FILEBACKED_AB_EXACT_REPEATABILITY_V0_1'):
