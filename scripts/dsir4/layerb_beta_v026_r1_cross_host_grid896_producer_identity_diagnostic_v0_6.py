#!/usr/bin/env python3
"""Response-blind cross-host GRID896 content-addressing diagnostic successor v0.6.

Implementation successor only. This file does not authorize execution or any
Layer-B science. V0.6 prospectively closes the terminal V0.5 executing-workflow
identity defect by fail-closed binding the current Actions run to the exact
canonical promoted workflow path and promoted workflow ID before any lane
operation and revalidating that identity in aggregate/decision.

All surviving V0.5 one-shot, first-parent chronology, binary64/hash/negative-
control, artifact-provenance, classifier-precedence, exact-32-lane and no-science
controls are preserved. A byte-identical alias workflow at another path must be
rejected as INVALID_DIAGNOSTIC_PROVENANCE.
"""
...
SCHEMA = "LAYERB_BETA_V0_26_R1_CROSS_HOST_GRID896_PRODUCER_IDENTITY_DIAGNOSTIC_EXECUTOR_V0_6"
LANE_RECEIPT_SCHEMA = "LAYERB_BETA_V0_26_R1_CROSS_HOST_GRID896_PRODUCER_IDENTITY_DIAGNOSTIC_LANE_RECEIPT_V0_6"
DECISION_SCHEMA = "LAYERB_BETA_V0_26_R1_CROSS_HOST_GRID896_PRODUCER_IDENTITY_DIAGNOSTIC_DECISION_V0_6"

ROOT = Path(__file__).resolve().parents[2]
EXECUTOR_PATH = Path("scripts/dsir4/layerb_beta_v026_r1_cross_host_grid896_producer_identity_diagnostic_v0_6.py")
WORKFLOW_CANDIDATE_PATH = Path("docs/dsir4/candidates/workflows/layerb-beta-v026-r1-cross-host-grid896-producer-identity-diagnostic-v0-6.yml")
ACTIVE_WORKFLOW_PATH = Path(".github/workflows/layerb-beta-v026-r1-cross-host-grid896-producer-identity-diagnostic-v0-6.yml")
...
IMPLEMENTATION_AUTHORITY_PATH = Path("docs/dsir4/authority/LAYERB_BETA_V0_26_R1_CROSS_HOST_GRID896_PRODUCER_IDENTITY_DIAGNOSTIC_IMPLEMENTATION_STATIC_AUDIT_AUTHORITY_V0_6.json")
LAUNCH_MARKER_PATH = Path("docs/dsir4/launch/LAYERB_BETA_V0_26_R1_CROSS_HOST_GRID896_PRODUCER_IDENTITY_DIAGNOSTIC_V0_6.launch.json")
...
def current_actions_workflow_identity() -> Dict[str, Any]:
    """Fail-closed bind this run to the exact canonical promoted workflow."""
    api = os.environ["GITHUB_API_URL"]
    repo = os.environ["GITHUB_REPOSITORY"]
    run_id = int(os.environ["GITHUB_RUN_ID"])
    token = os.environ.get("GITHUB_TOKEN", "")
    run = api_json(f"{api}/repos/{repo}/actions/runs/{run_id}", token)

    expected_path = ACTIVE_WORKFLOW_PATH.as_posix()
    if int(run.get("id", 0) or 0) != run_id:
        raise PermissionError("current Actions run id is not self-consistent")
    if int(run.get("run_number", 0) or 0) != int(os.environ["GITHUB_RUN_NUMBER"]):
        raise PermissionError("current Actions run_number API/environment mismatch")
    if int(run.get("run_attempt", 0) or 0) != int(os.environ["GITHUB_RUN_ATTEMPT"]):
        raise PermissionError("current Actions run_attempt API/environment mismatch")
    if run.get("event") != "push":
        raise PermissionError("current Actions run event is not push")
    if run.get("head_sha") != os.environ["GITHUB_SHA"]:
        raise PermissionError("current Actions run head_sha does not equal GITHUB_SHA")
    if run.get("head_branch") != "main":
        raise PermissionError("current Actions run head_branch is not main")
    if run.get("path") != expected_path:
        raise PermissionError(
            f"executing workflow path {run.get('path')!r} != canonical active path {expected_path!r}"
        )

    workflow_id = int(run.get("workflow_id", 0) or 0)
    if workflow_id <= 0:
        raise PermissionError("current Actions run lacks a positive workflow_id")

    expected_workflow_ref = f"{repo}/{expected_path}@refs/heads/main"
    actual_workflow_ref = os.environ.get("GITHUB_WORKFLOW_REF", "")
    if actual_workflow_ref != expected_workflow_ref:
        raise PermissionError(
            f"GITHUB_WORKFLOW_REF {actual_workflow_ref!r} != {expected_workflow_ref!r}"
        )

    workflow = api_json(f"{api}/repos/{repo}/actions/workflows/{workflow_id}", token)
    if int(workflow.get("id", 0) or 0) != workflow_id:
        raise PermissionError("workflow registry id mismatch")
    if workflow.get("path") != expected_path:
        raise PermissionError("workflow registry path does not equal canonical active path")
    if workflow.get("state") != "active":
        raise PermissionError("canonical workflow is not active in Actions registry")

    return {
        "executing_workflow_path": expected_path,
        "executing_workflow_id": workflow_id,
        "github_workflow_ref": actual_workflow_ref,
        "workflow_registry_path": workflow.get("path"),
        "workflow_registry_state": workflow.get("state"),
        "workflow_identity_bound_to_current_run": True,
    }


def verify_launch_authorization(executor_blob: str, workflow_blob: str) -> Dict[str, Any]:
    ...
    run_workflow = current_actions_workflow_identity()

    promoted_workflow_id = authority.get("promoted_workflow_id")
    if not isinstance(promoted_workflow_id, int) or promoted_workflow_id <= 0:
        raise PermissionError("implementation authority lacks positive promoted_workflow_id")
    ...
    required_authority = {
        ...
        "promoted_workflow_path": ACTIVE_WORKFLOW_PATH.as_posix(),
        "executing_workflow_identity_binding_required": True,
        "alias_workflow_execution_forbidden": True,
    }
    ...
    if run_workflow["executing_workflow_path"] != authority["promoted_workflow_path"]:
        raise PermissionError("current run workflow path is not authority-bound")
    if run_workflow["executing_workflow_id"] != promoted_workflow_id:
        raise PermissionError("current run workflow id is not authority-bound")

    parent_authority_bytes = git_show_bytes("HEAD^", IMPLEMENTATION_AUTHORITY_PATH)
    if git_blob_sha1(parent_authority_bytes) != authority_blob:
        raise PermissionError("terminal implementation authority did not preexist unchanged in HEAD^")
    ...
    if parent_active_blob != workflow_blob or parent_active_blob != active_workflow_blob:
        raise PermissionError("exact promoted active workflow did not preexist unchanged in HEAD^")
    ...
    required_marker = {
        ...
        "promoted_workflow_path": ACTIVE_WORKFLOW_PATH.as_posix(),
        "promoted_workflow_id": promoted_workflow_id,
    ...
    return {
        ...
        "authority_promoted_workflow_id": promoted_workflow_id,
        **run_workflow,
    }
...
def discover_job_id(expected_job_name: str) -> int:
    ...
    token = os.environ.get("GITHUB_TOKEN", "")
    payload = api_json(
        f"{api}/repos/{repo}/actions/runs/{run_id}/jobs?filter=all&per_page=100", token
    )
...
def list_run_artifacts() -> Tuple[List[Dict[str, Any]], int]:
    ...
    run_id = int(os.environ["GITHUB_RUN_ID"])
    token = os.environ.get("GITHUB_TOKEN", "")
    payload = api_json(f"{api}/repos/{repo}/actions/runs/{run_id}/artifacts?per_page=100", token)
...
def index_present_artifacts(
    ...
    return by_name, sorted(expected_names - actual_names)
...
def validate_receipt_provenance(
    ...
    mismatches = [key for key, value in expected_pairs.items() if receipt.get(key) != value]
    if mismatches:
        raise ProvenanceInvalid(f"receipt provenance mismatch for {lane_id}: {mismatches}")

    outcome = receipt.get("lane_outcome")
    if outcome not in {PASS, FAIL, BLOCKED_OBJECT}:
        raise ProvenanceInvalid(f"unexpected or invalid lane outcome for {lane_id}: {outcome!r}")
    return str(outcome)
...
def fail_predicate_established(receipt: Dict[str, Any]) -> Tuple[bool, bool]:
    ...
    if receipt.get("consumer_operation") != "STRUCT_DECODE_LE_IEEE754_BINARY64_THEN_STRUCT_RESERIALIZE_LE_WITHOUT_ARITHMETIC":
...
def aggregate_command(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        missing_lane_ids = [lane for lane in LANE_IDS if expected_artifact_name(lane) in missing_name_set]
        ...
        for lane_id in LANE_IDS:
            artifact = by_name.get(expected_artifact_name(lane_id))
            if artifact is None:
                continue
            receipt, artifact_provenance = receipt_from_artifact(artifact, lane_id)
        ...
        elif receipts_seen == 32 and len(decision["artifact_provenance"]) == 32 and all_present_pass:
...
def parser() -> argparse.ArgumentParser:
    ...
    aggregate = sub.add_parser(
        "aggregate", help="validate all present frozen lane artifacts before terminal classification"
    )
